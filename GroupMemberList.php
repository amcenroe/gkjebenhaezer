...
// Main select query
$sSQL = "SELECT per_ID, per_FirstName, per_MiddleName, per_LastName, per_Title, per_Suffix, per_Address1, per_Address2, 
		per_City, per_State, per_Zip, per_HomePhone, per_Country, per_Email,
			per_cls_id , per_birthyear , per_birthmonth, fam_ID, 
		fam_Address1, fam_Address2, fam_City, fam_State, fam_Zip, fam_Country, fam_HomePhone, fam_Email, lst_OptionName
			FROM person_per
...
$sSQL .= " ORDER BY family_fam.fam_ID , person_per.per_cls_id, person_per.per_birthyear, person_per.per_birthmonth, lst_OptionSequence ASC,per_LastName";
...
if (!$bPrintView) {
	...
	echo "<option value=\"0\"  selected>" . gettext("Perlihatkan Semuanya") . "</option>";
	...
	if ($Total > $iPerPage)
	{
		...
		if ($Result_Set < $Total && $Result_Set > 0)
		{
			...
			echo "<a href=\"GroupMemberList.php?Result_Set=$thisLinkResult&GroupID=$iGroupID&Sort=$iSort&Letter=$sLetter&ShowGSP=$bShowGSP\">". gettext("Hal Sebelumnya") . "</A>&nbsp;&nbsp;";
		}
		...
		if ($Result_Set >= 0 && $Result_Set < $Total)
		{
			...
			if ($thisLinkResult<$Total)
				echo "&nbsp;&nbsp;<a href=\"GroupMemberList.php?Result_Set=$thisLinkResult&GroupID=$iGroupID&Sort=$iSort&Letter=$sLetter&ShowGSP=$bShowGSP\">" . gettext("Hal Berikutnya") . "</a>";
		}
		...
		<select class=\"SmallText\" name=\"Number\">
			...
			<option value=\"50\" $sLimit100>100</option>
		</select>&nbsp;
		<input type=\"submit\" class=\"icTinyButton\" value=\"". gettext("Lihat") ."\">
		</form>";

		echo "</div><div align=\"center\">";
		echo "<a href=\"GroupMemberList.php?PrintView=1&GroupID=$iGroupID&Sort=$iSort&Letter=$sLetter&ShowGSP=$bShowGSP\" target=\"_top\">" . gettext("Cetak Daftar Ini") . "</a>";
	}
