...
/*******************************************************************************
 *
 *  filename    : AsetPeminjamEditor.php
...
//Set the page title
$sPageTitle = gettext("Editor Peminjam Aset GKJ Bekti");

//Get the AsetPinjamID out of the querystring
$iAsetPinjamID = FilterInput($_GET["AsetPinjamID"],'int');

// Security: User must have Add or Edit Records permission to use this form in those manners
// Clean error handling: (such as somebody typing an incorrect URL ?AsetPinjamID= manually)
if (strlen($iAsetPinjamID) > 0)
{
...
if (isset($_POST["AsetSubmit"]) || isset($_POST["AsetSubmitAndAdd"]))
{
	...
	$sAssetPinjamID = FilterInput($_POST["AssetPinjamID"]); 
	$sAssetID = FilterInput($_POST["AssetID"]); 
	$sAssetClass = FilterInput($_POST["AssetClass"]); 
	$sTglPinjam = FilterInput($_POST["TglPinjam"]); 
	$sTglKembali = FilterInput($_POST["TglKembali"]); 
	$sPeminjamInternal = FilterInput($_POST["PeminjamInternal"]); 
	$sPeminjamExternal = FilterInput($_POST["PeminjamExternal"]); 
	$sAlamatPeminjamExternal = FilterInput($_POST["AlamatPeminjamExternal"]); 
	$sJumlahPinjaman = FilterInput($_POST["JumlahPinjaman"]); 
	...
	//If no errors, then let's update...
		...
		// Existing Aset (update)
			...
			$sSQL .= "' WHERE AsetPinjamID = " . $iAsetPinjamID;
		...
		$sSQL = "INSERT INTO logger (date, userid, per_id, var1, var2)
			VALUES ('" . date("YmdHis") . "'," . $_SESSION['iUserID'].",'" . $iAsetPinjamID . "','" . $logvar1 . "','" . $logvar2 . "')";
		//Execute the SQL
			RunQuery($sSQL);

		// Check for redirection to another page after saving information: (ie. AsetPeminjamEditor.php?previousPage=prev.php?a=1;b=2;c=3)
		if ($previousPage != "") {
			$previousPage = str_replace(";","&",$previousPage) ;
			Redirect($previousPage . $iAsetPinjamID);
		}
		...
		else
		{
			...
			Redirect("AsetPeminjamEditor.php");
		}
	...
	if (strlen($iAsetPinjamID) > 0)
	{
		...
		$sSQL = "SELECT * FROM asetgkjbekti  WHERE AsetPinjamID = " . $iAsetPinjamID;
		$rsaset = RunQuery($sSQL);
		extract(mysql_fetch_array($rsaset));

		$sAsetPinjamID = $AsetPinjamID;
...
<form method="post" action="AsetPeminjamEditor.php?AsetPinjamID=<?php echo $iAsetPinjamID; ?>" name="AsetPeminjamEditor">
...
<table cellpadding="3" align="center" valign="top" >
	...
	<tr>
		<td <?php if ($numCustomFields > 0) echo "colspan=\"2\""; ?> align="center">
			...
			<input type="button" class="icButton" value="<?php echo gettext("Batal"); ?>" name="asetCancel" onclick="javascript:document.location='<?php if (strlen($iAsetPinjamID) > 0) 
{ echo "AsetView.php?AsetPinjamID=" . $iAsetPinjamID."&amp;$refresh"; 
