...
*  2006 Ed Davis
*  2009 Erwin Pratama for GKJ Bekasi Wilayah Timur
...
//	$sSQL = "SELECT name, ismenu, parent, content, uri, statustext, session_var, session_var_in_text, session_var_in_uri, url_parm_name, security_grp FROM menuconfig_mcf WHERE parent = '$menu' AND active=1 ".$security_matrix." ORDER BY sortorder";
	$sSQL = "SELECT name, ismenu, parent, content, 
	IF ((locate('?',uri)>0),CONCAT(uri,'&GID=',(RAND()+RAND())),CONCAT(uri,'?GID=',RAND(SUBSTRING(MD5(UUID()), 10, 7)))) as uri, 
	statustext, session_var, session_var_in_text, session_var_in_uri, url_parm_name, security_grp FROM menuconfig_mcf WHERE parent = '$menu' AND active=1 ".$security_matrix." ORDER BY sortorder";
//CONCAT(bulan,'?',SUBSTRING(MD5(UUID()), 10, 7)) as random 
...
global $MenuFirst, $sPageTitle, $sRootPath;
	...
	if ($sHeader) {
		...
		echo "<table width=100% bgcolor=white cellpadding=0 cellspacing=0 border=0><tr><td width=100%><A HREF=\"Menu.php?GID=$refresh\">";
			...
			<tr>
				...
				<td class="Search" width="50%">
					<form name="SelectFilter" method="get" action="<?php echo $sRootPath."/"; ?>SelectList.php">
						<input class="menuButton" style="font-size: 8pt; margin-top: 5px;" type="text" name="Filter" id="SearchText" <?php echo 'value="' . gettext("Lacak") . '"'; ?> onfocus="ClearFieldOnce(this);">
						<input name="mode" type="radio" value="person" <?php if (! $_SESSION['bSearchFamily']) echo "checked";?>><?php echo gettext("Jemaat"); ?><input type="radio" name="mode" value="family" <?php if ($_SESSION['bSearchFamily']) echo "checked";?>><?php echo gettext("Keluarga"); ?>
					</form>
				</td>
				<td class="Search" align="center">
					<?php /** if($_SESSION['bFinance']) echo gettext("Slip Deposit:") .
				": " . $_SESSION['iCurrentDeposit']; **/ ?>
				</td>
				<td class="Search" align="right">
				<?php /** echo gettext("Item di Keranjang") . ": " . count($_SESSION['aPeopleCart']);  **/ ?>
				</td>
				<td class="Search">&nbsp;&nbsp;&nbsp;</td>
				<td class="Search" align="right">
					<?php echo gettext("Pengguna:") . " " . $_SESSION['UserFirstName'] . " " . $_SESSION[' ']; 
					echo " - Tanggal : ";echo date2Ind(date("Y-m-d"),1);echo " - Minggu ke-" ;echo date("W", strtotime(date("Y-m-d")));
					echo "<br>";
