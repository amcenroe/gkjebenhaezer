...
/*******************************************************************************
*
*  filename    : /Include/FunctionsMove.php
...
function RemoveFromGroup($iPersonID, $iGroupID)
{
	...
	$sSQL = "SELECT custom_Field FROM person_custom_pindah_master WHERE type_ID = 9 AND custom_Special = " . $iGroupID;
	$result = RunQuery($sSQL);
	while ($aRow = mysql_fetch_array($result))
	{
		$sSQL = "UPDATE person_custom_pindah SET " . $aRow['custom_Field'] . " = NULL WHERE " . $aRow['custom_Field'] . " = " . $iPersonID;
		RunQuery($sSQL);
	}
}
...
function displayCustomField($type, $data, $special)
{
	...
	switch ($type)
	{
		...
		case 9:
			if ($data > 0) {
				$sSQL = "SELECT per_FirstName, per_LastName FROM person_pindah WHERE per_ID =" . $data;
				$rsTemp = RunQuery($sSQL);
...
function formCustomField($type, $fieldname, $data, $special, $bFirstPassFlag)
{
	...
	switch ($type)
	{
		...
		case 9:
			...
			$sSQL = "SELECT person_pindah.per_ID, person_pindah.per_FirstName, person_pindah.per_LastName
						FROM person2group2role_p2g2r
						LEFT JOIN person_pindah ON person2group2role_p2g2r.p2g2r_per_ID = person_pindah.per_ID
