...
//Set the page title
$sPageTitle = gettext("Editor Data Jemaat");
...
if (isset($_POST["PersonSubmit"]) || isset($_POST["PersonSubmitAndAdd"]))
{
	...
	{
		if ($iFamily < 1) {
			$sLastNameError = gettext("Silahkan masukkan nama Keluarga.");
			$bErrorFlag = true;
		} else {
	...
	// If they entered a full date, see if it's valid
		if (strlen($iBirthYear) > 0)
		{
			if ($iBirthYear == 0) { // If zero set to NULL
				...
				$sBirthYearError = gettext("Tahun SALAH: silahkan pilih antara 1901 sampai 2155");
				$bErrorFlag = true;
	...
	if (strlen($dMembershipDate) > 0)
	{
		...
		if ( $dateString === FALSE ) {
			$sMembershipDateError = "<span style=\"color: red; \">"
								. gettext("Tanggal Daftar SALAH") . "</span>";
			$bErrorFlag = true;
		} else {
	...
	if (strlen($sEmail) > 0)
	{
		if ( checkEmail($sEmail) == false ) {
			$sEmailError = "<span style=\"color: red; \">"
								. gettext("Alamat Email SALAH") . "</span>";
			$bErrorFlag = true;
		} else {
			$sEmail = $sEmail;
		}
...
//		}
		...
		// New Family (add)
		// Family will be named by the First Name.
		if ($iFamily == -1)
		{
			$sSQL = "INSERT INTO family_fam (fam_Name, fam_Address1, fam_Address2, fam_City, fam_State, fam_Zip, fam_Country, fam_HomePhone, fam_WorkPhone, fam_CellPhone, fam_Email, fam_DateEntered, fam_EnteredBy)
					VALUES ('" . $sFirstName . "','" . $sAddress1 . "','" . $sAddress2 . "','" . $sCity . "','" . $sState . "','" . $sZip . "','" . $sCountry . "','" . $sHomePhone . "','" . $sWorkPhone . "','". $sCellPhone . "','". $sEmail . "','" . date("YmdHis") . "'," . $_SESSION['iUserID'].")";
			//Execute the SQL
			RunQuery($sSQL);
			//Get the key back
			$sSQL = "SELECT MAX(fam_ID) AS iFamily FROM family_fam";
...
//			$sSQL = "UPDATE person_per SET per_Title = '" . $sTitle . "',per_FirstName = '" . $sFirstName . "',per_MiddleName = '" . $sMiddleName . "', per_LastName = '" . $sLastName . "', per_Suffix = '" . $sSuffix . "', per_Gender = " . $iGender . ", per_Address1 = '" . $sAddress1 . "', per_Address2 = '" . $sAddress2 . "', per_City = '" . $sCity . "', per_State = '" . $sState . "', per_Zip = '" . $sZip . "', per_Country = '" . $sCountry . "', per_HomePhone = '" . $sHomePhone . "', per_WorkPhone = '" . $sWorkPhone . "', per_CellPhone = '" . $sCellPhone . "', per_Email = '" . $sEmail . "', per_WorkEmail = '" . $sWorkEmail . "', per_BirthMonth = " . $iBirthMonth . ", per_BirthDay = " . $iBirthDay . ", " . "per_BirthYear = ". $iBirthYear. ", per_fam_ID = " . $iFamily . ", per_Fmr_ID = " . $iFamilyRole . ", per_cls_ID = " . $iClassification . ", per_MembershipDate = ";
			$sSQL = "UPDATE person_per SET per_Title = '" . $sTitle . "',per_FirstName = '" . $sFirstName . "',per_MiddleName = '" . $sMiddleName . "', per_LastName = '" . $sLastName . "', per_Suffix = '" . $sSuffix . "', per_Gender = " . $iGender . ", per_Address1 = '" . $sAddress1 . "', per_Address2 = '" . $sAddress2 . "', per_City = '" . $sCity . "', per_State = '" . $sState . "', per_Zip = '" . $sZip . "', per_Country = '" . $sCountry . "', per_HomePhone = '" . $sHomePhone . "', per_CellPhone = '" . $sCellPhone . "', per_Email = '" . $sEmail . "', per_WorkEmail = '" . $sWorkEmail . "', per_BirthMonth = " . $iBirthMonth . ", per_BirthDay = " . $iBirthDay . ", " . "per_BirthYear = ". $iBirthYear. ", per_fam_ID = " . $iFamily . ", per_Fmr_ID = " . $iFamilyRole . ", per_cls_ID = " . $iClassification . ", per_MembershipDate = ";
		...
		else if (isset($_POST["PersonSubmit"]))
		{
			...
			Redirect("PersonView.php?PersonID=" . $iPersonID."&amp;GID=$refresh");
		}
		else
		{
...
<table cellpadding="3" align="center" valign="top" >
	...
	<tr>
		<td <?php if ($numCustomFields > 0) echo "colspan=\"2\""; ?> align="center">
			...
			<input type="button" class="icButton" value="<?php echo gettext("Batal"); ?>" name="PersonCancel" onclick="javascript:document.location='<?php if (strlen($iPersonID) > 0) 
{ echo "PersonView.php?PersonID=" . $iPersonID."&amp;GID=$refresh"; 
} else {echo "SelectList.php?mode=person&amp;GID=$refresh"; 
} ?>';">
		</td>
	</tr>
	...
	<tr>
		<td <?php if ($numCustomFields > 0) echo "colspan=\"2\""; ?> class="SmallText" align="center">
			<?php echo gettext("Data yang dalam warna"); ?> <span style="color: red;"><?php echo gettext("merah"); ?></span> <?php echo gettext("mempunyai hubungan dengan data Keluarga."); ?>
		</td>
	</tr>

	<tr>
		<td colspan="3" class="SmallText" align="center" >
<?php if ($iPersonID < 1) 

{ 
				echo "<blink><b><font color=\"red\"> 
	
				PERHATIAN!!! 
				</font>
				</b></blink> Sebelum Anda menambahkan Data Baru, <font color=\"red\">PASTIKAN </font>anda MENCARI dahulu data yang akan ditambahkan. Untuk memastikan TIDAK ADA data yang <font color=\"red\">DUPLIKAT</font>";
...
?>
	...
	<?php /**
	**		<tr>
	**			<td class="LabelColumn" <?php addToolTip("Examples: Mr., Mrs., Dr., Rev."); ?>><?php echo gettext("Titel:"); ?></td>
	...
	**/ ?>
	...
	<tr>
		<td class="LabelColumn"><?php echo gettext("No Reff. Gereja Asal:"); ?></td>
	...
	<tr>
		<td class="LabelColumn"><?php echo gettext("Nama Jemaat:"); ?></td>
...
**/ ?>
			...
			<tr>
							<td class="LabelColumn" <?php addToolTip("Use drop down-menus to select the birth date. If the year is not known, you can still include the date (for birthday reference), although age will not be calculated."); ?>><?php echo gettext("Tempat/Tanggal Lahir:"); ?></td>
							<td class="TextColumn">
			<input type="text" name="WorkEmail" value="<?php echo htmlentities(stripslashes($sWorkEmail),ENT_NOQUOTES, "UTF-8"); ?>" size="20" maxlength="60"><font color="red"><?php echo $sWorkEmailError ?></font>
								...
								<select name="BirthMonth">
																	<option value="0" <?php if ($iBirthMonth == 0) { echo "selected"; } ?>><?php echo gettext(" - "); ?></option>
							...
							<font color="red"><?php echo $sBirthDateError ?></font>
								...
								<select name="BirthYear">
									...
									{
										if ($x < 10) { $sBirthYear = "0" . $x; } else { $sBirthYear = $x; }
									?>
										<option value="<?php echo $sBirthYear ?>" <?php if ($iBirthYear == $x) {echo "selected"; } ?>><?php echo $x ?></option>
									<?php } ?>
								</select>
							<font color="red"><?php echo $sBirthYearError ?></font>

			<?php /*

							<input type="text" name="BirthYear" value="<?php echo $iBirthYear ?>" maxlength="4" size="5"><font color="red"><?php echo $sBirthYearError ?></font><br><font size="1"><?php echo gettext("Masukkan dengan format 4 digit."); ?></font>
			...
			<?php /* ?>
							 <input type="text" name="BirthYear" value="<?php echo $iBirthYear ?>" maxlength="4" size="5"><font color="red"><br><?php echo $sBirthYearError ?></font><br><font size="2"><?php echo gettext("Silahkan dikosongkan jika Umur tidak mau ditampilkan."); ?></font>
...
<?php */ ?>
			...
			<tr>
				<td class="LabelColumn" <?php addToolTip("Jika merupakan anggota dari suatu keluarga, Silahkan pilih keluarga."); ?>><?php echo gettext("Keluarga:"); ?></td>
				<td class="TextColumn">
					<select name="Family" size="18">
						<option value="0" selected><?php echo gettext("Tidak Diketahui"); ?></option>
						<option value="-1"><?php echo gettext("Buat Keluarga Baru(Dengan Nama)"); ?></option>
						...
						while ($aRow = mysql_fetch_array($rsFamilies))
						{
							...
							echo ">" . $fam_Name . "&nbsp;" . FormatAddressLine($fam_Address1,"","");
			...
			<tr>
				<td class="LabelColumn" <?php addToolTip("Silahkan pilih Peran dalam Keluarga. Jika belum terdaftar dalam keluarga manapun,jangan pilih peran."); ?>><?php echo gettext("Status di Keluarga:"); ?></td>
				<td class="TextColumnWithBottomBorder">
					<select name="FamilyRole">
						<option value="0"><?php echo gettext("Tidak Diketahui"); ?></option>
...
<?php if (!$bHidePersonAddress) { /* Person Address can be hidden - General Settings */ ?>
			<tr>
							<td colspan="2" align="center"><h4><?php echo gettext("Silahkan Isikan Alamat jika alamat tidak sama dengan alamat keluarga"); ?></h4></td>
			</tr>

			<tr>
				<td class="LabelColumn" <?php addToolTip("Alamat utama dari individu jemaat.Silahkan dikosongi jika alamat individu jemaat sama dengan alamat keluarga."); ?>><?php if ($bFamilyAddress1) { echo "<span style=\"color: red;\">"; } ?><?php echo gettext("Alamat1:"); ?></span></td>
			...
			<tr>
				<td class="LabelColumn" <?php addToolTip("Informasi tambahan jika diperlukan. jika alamat individu jemaat sama dengan alamat keluarga."); ?>><?php if ($bFamilyAddress2) { echo "<span style=\"color: red;\">"; } ?><?php echo gettext("Alamat2:"); ?></span></td>
			...
			<tr>
				<td class="LabelColumn" <?php addToolTip("Either use the drop-down menu (for the US) or text box. If the state does not differ from the family, leave this field blank."); ?>><?php if ($bFamilyState) { echo "<span style=\"color: red;\">"; } ?><?php echo gettext("Propinsi:"); ?></span></td>
				<td class="TextColumn">
...
<?php } ?>
			...
			<tr>
				<td class="LabelColumn" <?php addToolTip("Format: xxx-xxx-xxxx Ext. xxx.<br>If the Home Phone does not differ from the family, leave this field blank."); ?>>
					<?php
					if ($bFamilyHomePhone)
						echo "<span style=\"color: red;\">". gettext("Telp.Rumah:") ."</span>";
					else
						echo gettext("Telp.Rumah:");
					?>
				</td>
				<td class="TextColumn">
		...
		**			<?php
		**			if ($bFamilyWorkPhone)
		**				echo "<span style=\"color: red;\">" . gettext("Telp.Kantor:") . "</span>";
		**			else
		**				echo gettext("Telp.Kantor:");
		**			?>
		**
		** </td>
		**
		...
		**/ ?>
			...
			<tr>
				<td class="LabelColumn" <?php addToolTip("Format: xxx-xxx-xxxx Ext. xxx.<br>If the Mobile Phone does not differ from the family, leave this field blank."); ?>>
					<?php
					if ($bFamilyCellPhone)
						echo "<span style=\"color: red;\">" . gettext("Handphone:") . "</span>";
					else
						echo gettext("Handphone:");
					?>
				</td>
				<td class="TextColumn">
...
<?php } ?>
			<tr>
				<td class="LabelColumn" <?php addToolTip("Format: YYYY-MM-DD<br>or enter the date by clicking on the calendar icon to the right."); ?>><?php echo gettext("Tanggal Daftar:"); ?></td>
			...
			<tr>
				<td class="LabelColumn" <?php addToolTip("Select the appropriate classification. These can be set using the classification manager in admin."); ?>><?php echo gettext("Klasifikasi:"); ?></td>
				<td class="TextColumnWithBottomBorder">
					<select name="Classification">
						...
						while ($aRow = mysql_fetch_array($rsClassifications))
						{
							...
							echo ">" . $lst_OptionName . " ($lst_OptionID)&nbsp;";
						}
						?>
		...
		<?php if ($numCustomFields > 0) { ?>
			...
			<table cellpadding="3">
				<tr>
					<td colspan="2" align="center"><h3><?php echo gettext("Data Pelengkap"); ?></h3></td>
				</tr>
				<?php
				mysql_data_seek($rsCustomFields,0);
