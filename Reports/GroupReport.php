...
class PDF_Directory extends ChurchInfoReport
{
	...
	function Header()
	{
		if ($this->PageNo() == 1)
		{
			...
			$this->Cell(10);
			//Framed title
			$sTitle = gettext("Buku Alamat - Kelompok ")  . $sGroupName ;
			if (strlen($sRoleName))
				$sTitle .= " (" . $sRoleName . ")";
...
*/
	...
	while ($aRow = mysql_fetch_array($rsRecords))
	{
		...
		if (isset($_POST['HomePhoneEnable']) && strlen($sHomePhone)) {
			...
			$OutStr .= "  " . gettext("Telepon") . ": " . $TempStr . "\n";
		}
		...
		if (isset($_POST['CellPhoneEnable']) && strlen($sCellPhone)) {
			...
			$OutStr .= "  " . gettext("Handphone") . ": " . $TempStr . "\n";
		}
...
if ($iPDFOutputType == 1)
	$pdf->Output("GKJBekasi_wiltimur_BukuAlamat-" . date("Ymd-Gis") . ".pdf", true);
else
	$pdf->Output();
?>
