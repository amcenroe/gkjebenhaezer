...
if (($previous_id > 0)) {
    $previous_link_text = "<a class=\"SmallText\" href=\"PersonView.php?PersonID=$previous_id\"><img border=0 src=\"Images/Icons/ico_prev.png\"  width=\"30\" height=\"30\"  > " . gettext("Sebelumnya") . "</a>";
}

if (($next_id > 0)) {
    $next_link_text = "<a class=\"SmallText\" href=\"PersonView.php?PersonID=$next_id\"><img border=0 src=\"Images/Icons/ico_next.png\"   width=\"30\" height=\"30\"   >" . gettext("Berikutnya") . "</a>";
}
...
{
$sPageTitle = gettext("Detail Jemaat - No.Induk: $per_ID$per_fam_ID$per_Gender$per_fmr_ID ");
...
<a href="PrintView.php?PersonID=<?php echo $per_ID; ?>" class="SmallText" target="_blank"><img border=0 src="Images/Icons/ico_print.png"  width=\"30\" height=\"30\"  >  <?php echo gettext("Cetak"); ?></a>
...
echo " | <a class=\"SmallText\" href=\"SelectList.php?mode=person\"><img border=0 src=\"Images/Icons/ico_list.png\"  width=\"30\" height=\"30\"  >" .
...
?>
  ...
  <?php
    ...
    if ($fam_ID != "") {
      ...
      echo gettext(" dari <br> Kelg.") . " <a href=\"FamilyView.php?FamilyID=" . $fam_ID . "\">" . $fam_Name . "<img border=0 src=\"Images/Icons/ico_kel.png\" align=\"middle\"  width=\"30\" height=\"30\"  ></a>" . gettext(" ") . "</font><br><br>";
    }
    else
...
//    move_uploaded_file($_FILES['SuratBaptisDewasa']['tmp_name'], "Images/BaptisDewasa/SBD" . $iPersonID . ".jpg");
        ...
        $SuratBaptisDewasaFile = "Images/BaptisDewasa/thumbnails/SBD" . $iPersonID . ".jpg";
            if (file_exists($SuratBaptisDewasaFile))
            {
                ...
                // Some old / M$ browsers can't handle PNG's correctly.
                if ($bDefectiveBrowser)
                    echo '<img border="0" src="Images/NoSuratBaptisDewasa.gif"  width="60" height="80"  ><br><br><br>';
                else
                    echo '<img border="0" src="Images/NoSuratBaptisDewasa.png"  width="60" height="80"  ><br><br><br>';
    ...
    $SuratSidhiFile = "Images/Sidhi/thumbnails/SS" . $iPersonID . ".jpg";
		...
		echo $bOkToEdit;
              if ($bOkToEdit) {
                ...
                // Some old / M$ browsers can't handle PNG's correctly.
                if ($bDefectiveBrowser)
                    echo '<img border="0" src="Images/NoSuratSidhi.gif"  width="60" height="80"  ><br><br><br>';
                else
                    echo '<img border="0" src="Images/NoSuratSidhi.png"  width="60" height="80"  ><br><br><br>';
...
function upload_berkas ($berkas, $kFileHeader, $diedit, $defBrowser) {
$bDefectiveBrowser = $defBrowser;
$bOkToEdit = $diedit;
$iPersonID = FilterInput($_GET["PersonID"],'int');
$iSurat= Surat;
$iErr= Error;
$iFile = File;
$kSurat = "$iSurat$berkas";
$kSuratError = "$iSurat$berkas$iErr";
$kFolder = "$berkas";
$kFile = "$iSurat$berkas$iFile";

// Upload Surat Baptis Sidhi
  if ( isset($_POST["Upload'.$kFolder.'"]) && ($_SESSION['bAddRecords'] || $bOkToEdit) ) {
    if ($_FILES[".$kSurat."]['name'] == "") {
      $kSuratError = gettext("Tidak Ada SuratSidhi yang di upload.");
    } elseif ($_FILES[".$kSurat."]['type'] != "image/pjpeg" && $_FILES[".$kSurat."]['type'] != "image/jpeg") {
      $kSuratError = gettext("Hanya Format JPEG yang bisa di upload.");
  } else {
  // Create the thumbnail used by PersonView
            chmod ($_FILES[".$kSurat."]['tmp_name'], 0777);
    $srcImage=imagecreatefromjpeg($_FILES[".$kSurat."]['tmp_name']);
    $src_w=imageSX($srcImage);
    $src_h=imageSY($srcImage);
  // Calculate thumbnail's height and width (a "maxpect" algorithm)
    $dst_max_w = 150;
    ...
    if ($dst_max_w >= $dst_max_h) {
      ...
      $dstImage=ImageCreateTrueColor($thumb_w,$thumb_h);
      imagecopyresampled($dstImage,$srcImage,0,0,0,0,$thumb_w,$thumb_h,$src_w,$src_h);
      imagejpeg($dstImage, "Images/".$kFolder."/thumbnails/".$kFileHeader.$iPersonID.".jpg");
  ...
  if ($dst_max_w >= $dst_max_h) {
    ...
    $dstImage=ImageCreateTrueColor($thumb_w,$thumb_h);
           imagecopyresampled($dstImage,$srcImage,0,0,0,0,$thumb_w,$thumb_h,$src_w,$src_h);
    imagejpeg($dstImage, "Images/".$kFolder."/".$kFileHeader.$iPersonID.".jpg");
    imagedestroy($dstImage);
    imagedestroy($srcImage);

    }
    } elseif (isset($_POST["Delete'.$kSurat.'"]) && $_SESSION['bDeleteRecords']) {
    unlink("Images/".$kFolder."/".$kFileHeader.$iPersonID.".jpg");
    unlink("Images/".$kFolder."/thumbnails/".$kFileHeader.$iPersonID.".jpg");
    }

    $SuratFile = "Images/".$kFolder."/thumbnails/".$kFileHeader.$iPersonID.".jpg";
          if (file_exists($SuratFile))
          {
              echo '<a target="_blank" href="Images/'.$kFolder.'/'.$kFileHeader.$iPersonID.'.jpg"  >';
              echo '<img border="1" src="'.$SuratFile.'"  width="50" height="70"  ></a>';
              if ($bOkToEdit) {
                  echo '
                      <form method="post"
                      action="PersonView.php?PersonID=' . $iPersonID . '">
                      <br>
                      <input type="submit" class="icTinyButton"
                      value="Hapus ' . $kSurat . '"  name="Delete'.$kSurat.'">
                      </form>';
                    }
            } else {
                // Some old / M$ browsers can't handle PNG's correctly.
                if ($bDefectiveBrowser)
                    echo '<img border="0" src="Images/NoSurat.gif"  width="50" height="70"  ><br><br><br>';
                else
                    echo '<img border="0" src="Images/NoSurat.png"  width="50" height="70"  ><br><br><br>';

                if ($bOkToEdit) {
                    if (isset($kSuratError))
                        echo '<span style="color: red;">' . $kSuratError . '</span><br>';

                    echo '
                        <form method="post"
                        action="PersonView.php?PersonID=' . $iPersonID . '"
                        enctype="multipart/form-data">
                        <input class="icTinyButton" type="file" name="' . $kSurat . '" size="1" >
                        <input type="submit" class="icTinyButton"
                        value="Upload '.$kSurat.'" name="Upload'.$kSurat.'">
                        </form>';
                }
        }
...
?>
	...
	<?php
	upload_berkas(BaptisAnak,SBA,$bOkToEdit,$bDefectiveBrowser);   
	upload_berkas(BaptisDewasa,SBD,$bOkToEdit,$bDefectiveBrowser);   
	upload_berkas(Sidhi,SS,$bOkToEdit,$bDefectiveBrowser);   
	upload_berkas(AtestasiMasuk,ATIN,$bOkToEdit,$bDefectiveBrowser);   
	upload_berkas(AtestasiKeluar,ATOT,$bOkToEdit,$bDefectiveBrowser);   
...
<td width="75%" valign="top" align="left">
  ...
  <tr>
    <td align="center">
      ...
      <tr>
      <td class="TinyLabelColumn"><?php echo gettext("No.Reff.GKJ Bekasi"); ?></td>
    ...
    while ($aRow = mysql_fetch_array($rsAssignedGroups))
    {
      ...
      if ($_SESSION['bManageGroups']) {
        if ($grp_hasSpecialProps == 'true')
          ...
          echo "<td>&nbsp;</td>";
        echo "<td><input type=\"button\" class=\"icTinyButton\" value=\"" . gettext("Hapus") . "\" Name=\"remove\" onclick=\"GroupRemove(" . $grp_ID . ", " . $iPersonID . ");\" ></td>";
      }
      echo "</tr>";
