...
/*******************************************************************************
 *
 *  filename    : SuKepView.php
 *  description : Displays all the information about a single SK
...
// Get the SK ID from the querystring
$iSKID = FilterInput($_GET["SKID"],'int');

$iRemoveVO = FilterInput($_GET["RemoveVO"],'int');

$dSQL= "SELECT SKID FROM SuratKeputusan order by SKID";
...
if (($previous_id > 0)) {
    $previous_link_text = "<a class=\"SmallText\" href=\"SuKepView.php?SKID=$previous_id\"><img border=0 src=\"Images/Icons/ico_prev.png\"  width=\"30\" height=\"30\"  > " . gettext("Sebelumnya") . "</a>";
}

if (($next_id > 0)) {
    $next_link_text = "<a class=\"SmallText\" href=\"SuKepView.php?SKID=$next_id\"><img border=0 src=\"Images/Icons/ico_next.png\"   width=\"30\" height=\"30\"   >" . gettext("Berikutnya") . "</a>";
}


// Get this SK's data
$sSQL = "SELECT a.* 
FROM SuratKeputusan a
WHERE SKID = " . $iSKID;
...
gettext("Surat Keputusan : $NomorSurat2 ");
...
if ($_SESSION['bDeleteRecords']) { echo "<a class=\"SmallText\" href=\"SelectDelete.php?mode=sukep&SKID=" . $SKID . "\"><img border=0 src=\"Images/Icons/ico_del.png\"   width=\"30\" height=\"30\"   > " . gettext("Hapus") . "</a> | " ; }

?>
<a href="SuKepEditor.php?SKID=<?php echo $SKID; ?>" class="SmallText" ><img border=0 src="Images/Icons/ico_edit.png"  width=30 height=30  >  <?php echo gettext("Edit"); ?></a>
<?php

if ($next_link_text) {
  echo " | $next_link_text";
}
echo " | <a class=\"SmallText\" href=\"SelectListApp.php?mode=sukep\"><img border=0 src=\"Images/Icons/ico_list.png\"  width=\"30\" height=\"30\"  >" .
...
echo "<div class=\"LightShadedBox\"> ";
    ...
    if ( isset($_POST["UploadPhoto"]) && ($_SESSION['bAddRecords'] || $bOkToEdit) ) {
      if ($_FILES['Photo']['name'] == "") {
        ...
        // Create the thumbnail used by SKView
        ...
        imagejpeg($dstImage, "Images/SK/thumbnails/" . $iSKID . ".jpg");
...
//         imagedestroy($srcImage);
        ...
        $dstImage=ImageCreateTrueColor($thumb_w,$thumb_h);
            imagecopyresampled($dstImage,$srcImage,0,0,0,0,$thumb_w,$thumb_h,$src_w,$src_h);
        imagejpeg($dstImage, "Images/SK/" . $iSKID . ".jpg");
        imagedestroy($dstImage);
          imagedestroy($srcImage);
        ...
        //move_uploaded_file($_FILES['Photo']['tmp_name'], "Images/SK/" . $iSKID . ".jpg");
      }
    } elseif (isset($_POST["DeletePhoto"]) && $_SESSION['bDeleteRecords']) {
      unlink("Images/SK/" . $iSKID . ".jpg");
      unlink("Images/SK/thumbnails/" . $iSKID . ".jpg");
    }

    // Display photo or upload from file
    $photoFile = "Images/SK/thumbnails/" . $iSKID . ".jpg";
        if (file_exists($photoFile))
        {
            echo '<a target="_blank" href="Images/SK/' . $iSKID . '.jpg">';
            echo '<img border="1" src="'.$photoFile.'"  Images/NoSK.gif" width="128" height="126"   ></a>';
            if ($bOkToEdit) {
                echo '
                    <form method="post"
                    action="SuKepView.php?SKID=' . $iSKID . '">
                    <br>
            ...
            if ($bDefectiveBrowser)
                echo '<img border="0" src="Images/NoSK.gif" width="64" height="63"  ><br><br><br>';
            else
                echo '<img border="0" src="Images/NoSK.png" width="64" height="63"  ><br><br><br>';
...
<td width="80%" valign="top" align="left">
  ...
  <b><?php echo gettext("Informasi Surat Keputusan :  ")  ; ?></b>
  <b><?php echo gettext("$SKID - $NomorSurat2"); ?> </b>
  ...
  <tr>
    <td align="center">
      ...
      <tr>
      <td class="TinyLabelColumn"><?php echo gettext("Tanggal Kadaluwarsa"); ?></td>
      <td class="TinyTextColumn"><?php echo date2Ind($TanggalExp,1) ; ?></td>
      </tr>
      <tr>
      ...
      <td class="TinyTextColumn">
	  <table>
		<tr>	<td> Cetak Surat Keputusan: </td>
	  			<td><a target="_blank" href="PrintViewSuratKeputusan.php?Type=ISI&SKID=<?php echo $SKID ?>&Mode=1"><?php echo "Cetak dengan Kop Surat" ?></a> atau </td>
				<td><a target="_blank" href="PrintViewSuratKeputusan.php?Type=ISI&SKID=<?php echo $SKID ?>&Mode=3"><?php echo "Cetak polos" ?></a></td>
		</tr>
	  </table>
	  </td>
      </tr>
	  ...
	  <table>
		<tr>	<td> Cetak Surat Keputusan: </td>
	  			<td><a target="_blank" href="PrintViewSuratKeputusan.php?Type=LAMP&SKID=<?php echo $SKID ?>&Mode=1"><?php echo "Cetak dengan Kop Surat " ?> </a> atau </td>
	
				<td><a target="_blank" href="PrintViewSuratKeputusan.php?Type=LAMP&SKID=<?php echo $SKID ?>&Mode=3"><?php echo "Cetak Polos" ?></a></td>
