...
//$sqlquery = FilterInput($_GET["Status"]);
$sqlquery = $_POST["sqlpdf"];
$sqlquery3 = $_POST["sqlpdf3"];
...
function Header()
{
    ...
    if(!$this->headerset[$this->page]) {
        ...
        $this->SetXY(60,10);
        $this->Image('gkj_logo.jpg',30,3,32);
        //$this->Cell(50,1,$sqlquery3,0,1,1);
        $this->Cell(50,1,'GKJ Bekasi Timur',0,1,1);

        $this->SetXY(60,20);
        $this->Cell(50,1,'-----------------------------------',0,1,1);



        $this->SetY(($this->tMargin+10) - ($this->FontSizePt/$this->k)*2);
        ...
        foreach($this->colTitles as $col => $txt) {
            $this->SetXY($l,($this->tMargin+10));
            $this->MultiCell($this->tablewidths[$col], $this->FontSizePt,$txt);
            $l += $this->tablewidths[$col] ;
            $maxY = ($maxY < $this->getY()) ? $this->getY() : $maxY ;
        }
        $this->SetXY($this->lMargin,$this->tMargin+10);
        $this->setFillColor(200,200,200);
        $l = ($this->lMargin);
        foreach($this->colTitles as $col => $txt) {
            $this->SetXY($l,$this->tMargin+10);
            $this->cell($this->tablewidths[$col],$maxY-($this->tMargin+10),'',1,0,'L',1);
            $this->SetXY($l,$this->tMargin+10);
...
function Footer() {
    ...
    if(!$this->footerset[$this->page]) {
        ...
        $this->Cell(0,10,'Sistem Informasi GKJ Bekasi Timur - Halaman '.$this->PageNo().'/{nb}',0,0,'C');
        // set footerset
...
function morepagestable($lineheight=8) {
    ...
    for($i = $startpage; $i <= $maxpage; $i++) {
        $this->page = $i;
        $l = $this->lMargin;
        $t = ($i == $startpage) ? $startheight : $this->tMargin+10;
...
//$HeaderReport = "Daftar Warga Kelompok ";
$HeaderReport = $sqlquery3;

$attr=array('titleFontSize'=>10,'titleText'=>$HeaderReport);
//$attr=array('titleFontSize'=>14,'titleText'=>$HeaderReport);
// This is your query. It should be a 'SELECT' query.
// Reports are run over 'SELECT' querires generally.
//$sqlquery = " select per_ID as perId from person_per ";
//$sqlquery = " SELECT per_FirstName as Nama FROM person_per NATURAL JOIN person_custom LEFT JOIN family_fam ON per_fam_ID = family_fam.fam_ID WHERE 1 AND c4 =1 GROUP BY person_per.per_ID ORDER BY per_LastName, per_FirstName";
//$sqlquery = "SELECT per_FirstName as 'Nama Lengkap', fam_HomePhone as 'Telp Rumah', per_Cellphone as Handphone, a1.lst_OptionName as 'Hub Keluarga',
//							fam_Name as 'Keluarga', fam_Address1 as 'Alamat Lengkap', a2.lst_OptionName as 'Status'
//						FROM person_per
...
$pdf->mysql_report($sqlquery,false,$attr);
