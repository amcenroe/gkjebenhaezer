...
/*******************************************************************************
 *
 *  filename    : MasterAnggaranKlaimAsuransiEditor.php
...
//Set the page title
$sPageTitle = gettext("Daftar Master PosAnggaran Editor");

//Get the PosAnggaranID out of the querystring
$iPosAnggaranID = FilterInput($_GET["PosAnggaranID"],'int');

// Security: User must have Add or Edit Records permission to use this form in those manners
// Clean error handling: (such as somebody typing an incorrect URL ?PosAnggaranID= manually)
if (strlen($iPosAnggaranID) > 0)
{
...
if (isset($_POST["MasterPosAnggaranSubmit"]) || isset($_POST["MasterPosAnggaranSubmitAndAdd"]))
{
	...
	$sNamaPosAnggaran = FilterInput($_POST["NamaPosAnggaran"]);
	...
	//If no errors, then let's update...
		// New Data (add)
		if (strlen($iPosAnggaranID) < 1)
		{
			if (!$_SESSION['bFinance'] || strlen($sEnvelope) < 1)
				$sEnvelope = "NULL";

				
			$sSQL = "INSERT INTO MasterPosAnggaran ( 
			PosAnggaranID,
			KomisiID,
			Enable,
			NamaPosAnggaran,
	...
	//update the Baptis table
			$sSQL = "UPDATE MasterPosAnggaran SET 

	PosAnggaranID = '" . $sPosAnggaranID . "',
	KomisiID = '" . $sKomisiID . "',	
	Enable = '" . $sEnable . "',
	NamaPosAnggaran = '" . $sNamaPosAnggaran . "',
	Keterangan = '" . $sKeterangan . "',
	
			DateLastEdited = '" . date("YmdHis") . "',
			EditedBy = '" . $_SESSION['iUserID'] ;
				
			$sSQL .= "' WHERE PosAnggaranID = " . $iPosAnggaranID;
		...
		$sSQL = "INSERT INTO logger (date, userid, per_id, var1, var2)
			VALUES ('" . date("YmdHis") . "'," . $_SESSION['iUserID'].",'" . $iPosAnggaranID . "','" . $logvar1 . "','" . $logvar2 . "')";
		//Execute the SQL
			RunQuery($sSQL);

		// Check for redirection to another page after saving information: (ie. MasterAnggaranKlaimAsuransi.php?previousPage=prev.php?a=1;b=2;c=3)
		if ($previousPage != "") {
			$previousPage = str_replace(";","&",$previousPage) ;
			Redirect($previousPage . $iPosAnggaranID);
		}
		else if (isset($_POST["MasterPosAnggaranSubmit"]))
		{
			//Send to the view of this PendetaData
			Redirect("SelectListApp.php?mode=masterposangg&amp;$refresh");

		}
		else
		{
			//Reload to editor to add another record
			Redirect("MasterAnggaranKlaimAsuransi.php&amp;$refresh");
		}
	...
	if (strlen($iPosAnggaranID) > 0)
	{
		...
		$sSQL = "SELECT * FROM MasterPosAnggaran  WHERE PosAnggaranID = " . $iPosAnggaranID;
		$rsGereja = RunQuery($sSQL);
		extract(mysql_fetch_array($rsGereja));
	...
	$sKomisiID = $KomisiID;	
	$sEnable = $Enable;
	$sNamaPosAnggaran = $NamaPosAnggaran;
...
<form method="post" action="MasterAnggaranKlaimAsuransi.php?PosAnggaranID=<?php echo $iPosAnggaranID; ?>" name="MasterAnggaranKlaimAsuransi">
...
<table cellpadding="3" align="center" valign="top" >
	...
	<tr>
		<td <?php if ($numCustomFields > 0) echo "colspan=\"2\""; ?> align="center">
			<input type="submit" class="icButton" value="<?php echo gettext("Simpan"); ?>" name="MasterPosAnggaranSubmit">
			<?php if ($_SESSION['bAddRecords']) { echo "<input type=\"submit\" class=\"icButton\" value=\"" . gettext("Simpan dan Tambah") . "\" name=\"MasterPosAnggaranSubmitAndAdd\">"; } ?>
			<input type="button" class="icButton" value="<?php echo gettext("Batal"); ?>" name="pakCancel" onclick="javascript:document.location='<?php if (strlen($iPosAnggaranID) > 0) 
{ echo "SelectListApp.php?mode=masterposangg&amp;$refresh"; 
} else {echo "SelectListApp.php?mode=masterposangg&amp;$refresh"; 
} ?>';">
		</td>
	</tr>
	...
	<tr>
		<td class="LabelColumn" ><?php echo gettext("Nama PosAnggaran"); ?></td>
		<td class="TextColumn" colspan="3"><input type="text" name="NamaPosAnggaran" size=80 id="NamaPosAnggaran" value="<?php echo htmlentities(stripslashes($sNamaPosAnggaran),ENT_NOQUOTES, "UTF-8"); ?>"><br>
		<font color="red"><?php echo $sNamaPosAnggaranError ?></font></td>
	</tr>
	<tr>
