...
/*******************************************************************************
 *
 *  filename    : BaptisDewasaEditor.php
...
//Set the page title
$sPageTitle = gettext("Pendaftaran Baptis Dewasa");
...
if (isset($_POST["BaptisSubmit"]) || isset($_POST["BaptisSubmitAndAdd"]))
{
	...
	//If no errors, then let's update...
		...
		if (strlen($iBaptisID) < 1)
		{
			...
			$sSQL = "INSERT INTO baptisdewasagkjbekti ( 
	...
	//update the Baptis table
			$sSQL = "UPDATE baptisdewasagkjbekti SET 
		...
		//	echo $sSQL;
			...
			$logvar1 = "Edit";
			$logvar2 = "Update Baptis Dewasa Data";
		}
		...
		// Check for redirection to another page after saving information: (ie. BaptisDewasaEditor.php?previousPage=prev.php?a=1;b=2;c=3)
		if ($previousPage != "") {
		...
		else if (isset($_POST["BaptisSubmit"]))
		{
			...
			Redirect("SelectListApp.php?mode=BaptisDewasa&amp;GID=$refresh");

		}
		else
		{
			//Reload to editor to add another record
			Redirect("BaptisDewasaEditor.php");
		}
...
left join person_per c ON (b.fam_id = c.per_fam_id AND c.per_fmr_id = 1 AND c.per_gender = 1)
left join person_per d ON (b.fam_id = d.per_fam_id AND d.per_fmr_id = 2 AND d.per_gender = 2)
where a.per_cls_id <3 and
(x.c1 is null or x.c1='0000-00-00' ) and
(x.c26 is null or x.c26='0000-00-00' )and
(x.c2 is null or x.c2='0000-00-00' ) and
(x.c27 is null or x.c27='0000-00-00' )and
(x.c18 is null or x.c18='0000-00-00' ) and
(x.c28 is null or x.c28='0000-00-00' )
order by a.per_firstname ";
}
...
<form method="post" action="BaptisDewasaEditor.php?BaptisID=<?php echo $iBaptisID; ?>" name="BaptisEditor">
...
{ echo "SelectListApp.php?mode=BaptisDewasa&amp;GID=$refresh"; 
} else {echo "SelectListApp.php?mode=BaptisDewasa&amp;GID=$refresh"; 
} ?>';">
		</td>
	</tr>
	...
	<tr>
			<td colspan="4" align="center"><h3><?php echo gettext("Isikan Data dibawah jika Titip Pelayanan di Gereja lain"); ?></h3></td>
	</tr>	
	<tr>
		<td class="LabelColumn"><?php echo gettext("Dilayani di Gereja :"); ?></td>
		<td class="TextColumnWithBottomBorder">
	...
	<tr>
		<td class="LabelColumn"><?php echo gettext("Nama Gereja(Jika bukan GKJ):"); ?></td>
		<td class="TextColumn"><input type="text" name="TempatBaptis2" id="TempatBaptis2" value="<?php echo htmlentities(stripslashes($sTempatBaptis2),ENT_NOQUOTES, "UTF-8"); ?>"><br><font color="red"><?php echo $sTempatBaptis2Error ?></font></td>
	</tr>
...
<?php
		$logvar1 = "Edit";
		$logvar2 = "Pendaftaran Baptis Dewasa Editor";
		//Update Logger
		$sSQL = "INSERT INTO logger (date, userid, per_id, var1, var2)
			VALUES ('" . date("YmdHis") . "'," . $_SESSION['iUserID'].",'" . $iBaptisID . "','" . $logvar1 . "','" . $logvar2 . "')";
		//Execute the SQL
