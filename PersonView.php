...
function QRCode($per_ID,$textvar){
$qr = new SocialQrCode ();
$qr->setType ( SocialQrCode::QRCODE_TYPE_PNG );
$qr->generate ( "http://www.gkjbekasitimur.org/datawarga/PersonView.php?PersonID=$per_ID" );
$qr->show ();
}
...
// Hapus Kelompok yg Lama
//GroupRemove(" . $grp_ID . ", " . $iPersonID . ")
//GroupMemberList.php?GroupID=" + Group + "&PersonToRemove=" + Person

	// Check if this group has special properties
//	$sSQL = "SELECT p2g2r_grp_ID as delGroupID FROM person2group2role_p2g2r WHERE p2g2r_per_ID = " . $iPersonID;
...
//	if ($bHasProp == 'true')
//	{
		$dsSQL = "DELETE FROM person2group2role_p2g2r WHERE p2g2r_per_ID = " . $iPersonID ;
		RunQuery($dsSQL);
//	}
	
//  RemoveFromGroup($iPersonID,$iGroupID);
// Assign Group yg Baru
  $iGroupID = FilterInput($_POST["GroupAssignID"],'int');
  AddToGroup($iPersonID,$iGroupID,0);

  $sSQL = "SELECT grp_Name, grp_Description FROM person2group2role_p2g2r INNER JOIN group_grp
  ...
  $sSQL = "UPDATE `person_per` SET `per_WorkPhone` = \"".$iRoleID."\" WHERE `per_ID` = ".$iPersonID."  LIMIT 1 ";
...
// QRCode($hasilGD[per_ID],$vartext) ;

if (($previous_id > 0)) {
    $previous_link_text = "<a class=\"SmallText\" href=\"PersonView.php?PersonID=$previous_id&GID=$refresh&\"><img border=0 src=\"Images/Icons/ico_prev.png\"  width=\"30\" height=\"30\"  > " . gettext("Sebelumnya") . "</a>";
}

if (($next_id > 0)) {
    $next_link_text = "<a class=\"SmallText\" href=\"PersonView.php?PersonID=$next_id&GID=$refresh&\"><img border=0 src=\"Images/Icons/ico_next.png\"   width=\"30\" height=\"30\"   >" . gettext("Berikutnya") . "</a>";
}
...
$sSQL = "SELECT a.*, family_fam.*, cls.lst_OptionName AS sClassName, fmr.lst_OptionName AS sFamRole, b.per_FirstName AS EnteredFirstName,
	  ...
	  LEFT JOIN person_custom e ON a.per_ID = e.per_ID 
...
// Get Family Relation
// Get the custom field data for this person.
$sSQL = "SELECT c16 as NamaAyah, c17 as NamaIbu FROM person_custom WHERE per_ID = " . $iPersonID;
$rsFamilyTree = RunQuery($sSQL);
extract(mysql_fetch_array($rsFamilyTree));

$sSQL = "SELECT a.per_ID as BapakID, a.per_fam_ID as KeluargaIDBpk FROM person_per a LEFT JOIN person_custom b ON a.per_ID = b.per_ID WHERE per_FirstName = '" . $NamaAyah . "' LIMIT 1";
$rsFamilyTreeAyah = RunQuery($sSQL);
if (mysql_num_rows(RunQuery($sSQL)) > 0){extract(mysql_fetch_array($rsFamilyTreeAyah));}else{}
$sSQL = "SELECT a.per_ID as IbuID, a.per_fam_ID as KeluargaIDIbu FROM person_per a LEFT JOIN person_custom b ON a.per_ID = b.per_ID WHERE per_FirstName = '" . $NamaIbu . "' LIMIT 1";
$rsFamilyTreeIbu = RunQuery($sSQL);
if (mysql_num_rows(RunQuery($sSQL)) > 0){extract(mysql_fetch_array($rsFamilyTreeIbu));}else{}
...
$sSQL = "SELECT grp_ID, grp_Name, grp_Description, grp_hasSpecialProps, role.lst_OptionName AS roleName
    FROM group_grp
...
{
$sPageTitle = gettext("[MENINGGAL] Detail Jemaat - No.Induk: $per_ID$per_fam_ID$per_Gender$per_fmr_ID ");
$_SESSION['StatusWarga'] = "Meninggal" ;
} elseif ($sClassName == "Pindah")
{
$sPageTitle = gettext("[PINDAH] Detail Jemaat - No.Induk: $per_ID$per_fam_ID$per_Gender$per_fmr_ID ");
$_SESSION['StatusWarga'] = "Pindah";
} elseif ($sClassName == "Warga NonAktif")
{
$sPageTitle = gettext("[<b><font color=\"red\"><blink>TIDAK AKTIF/DIKETAHUI</blink></font></b>] Detail Jemaat - No.Induk: $per_ID$per_fam_ID$per_Gender$per_fmr_ID ");
$_SESSION['StatusWarga'] = "Warga NonAktif";
} else
{
  $KodeJemaat = KodeNamaWarga($per_FirstName);
$sPageTitle = gettext("Detail Jemaat - No.Induk: $per_ID$per_fam_ID$per_Gender$per_fmr_ID Kode:$KodeJemaat");
$_SESSION['StatusWarga'] = "Aktif";
}
...
echo "<br>";
if ( empty($KeluargaIDBpk))
{echo "";}
elseif ( $KeluargaIDBpk==$KeluargaIDIbu ){
echo "<a href=\"FamilyView.php?FamilyID=" . $KeluargaIDBpk . "&amp;GID=$refresh \" class=\"SmallText\" target=\"_blank\"><img border=0 src=\"Images/Icons/familytree.png\"  ></a>";
}
//echo $BapakID . "/" . $KeluargaIDBpk . " - " . $IbuID . "/" . $KeluargaIDIbu ;
...
if ($bOkToEdit) {
  echo "<a class=\"SmallText\" href=\"PersonEditor.php?PersonID=" . $per_ID ."&amp;GID=$refresh
...
?>
<a href="VCardCreate.php?PersonID=<?php echo $per_ID; ?>" class="SmallText"><img border=0 src="Images/Icons/ico_vcard.png"  width=30 height=30  > <?php echo gettext("vCard"); ?></a> |
<a href="PrintView.php?PersonID=<?php echo $per_ID."&GID=$refresh&"; ?>" class="SmallText" target="_blank"><img border=0 src="Images/Icons/ico_print.png"  width=30 height=30  >  <?php echo gettext("Cetak Detail"); ?></a> | 
<a href="PrintViewKartuPersembahan.php?JenisKartu=BLN&B1=B1<?php echo $per_ID."&GID=$refresh&"; ?>" class="SmallText" target="_blank"><img border=0 src="Images/Icons/ico_printbulanan.png"  width=30 height=30  >  <?php echo gettext("Cetak Kartu BLN"); ?></a> | 
<a href="PrintViewKartuPersembahan.php?JenisKartu=PPPG&&B1=B1<?php echo $per_ID."&GID=$refresh&"; ?>" class="SmallText" target="_blank"><img border=0 src="Images/Icons/ico_printpppg.png"  width=30 height=30  >  <?php echo gettext("Cetak Kartu PPPG"); ?></a> | 
<script>
...
<a href="javascript:void(0);" onclick="PopupCenter('http://www.gkjbekasitimur.org/datawarga/webcamcapture/index_person.php?PersonID=<?=$per_ID?> ', 'myPop1',600,500);"><img border=0 src="Images/Icons/ico_webcam.jpg"  width=25 height=25  ><?php echo gettext("Webcam"); ?></a>



<?php /** | <a href="PersonView.php?PersonID=<?php echo $per_ID; ?>&AddToPeopleCart=<?php echo $per_ID; ?>" class="SmallText"><?php echo gettext("Cart"); ?></a>
**/ ?>
<?php
//if ($_SESSION['bAdmin'])
//{
...
echo " | <a class=\"SmallText\" href=\"SelectList.php?mode=person&GID=$refresh&\"><img border=0 src=\"Images/Icons/ico_list.png\"  width=\"30\" height=\"30\"  >" .
...
} elseif ($sClassName == "Tamu")
{
echo "<div class=\"LightShadedBoxTAMU\"> ";

} elseif ($sClassName == "Calon")
{
echo "<div class=\"LightShadedBoxCALON\"> ";

} elseif ($sClassName == "InAktif")
{
echo "<div class=\"LightShadedBoxINAKTIF\"> ";
...
?>
  ...
  <?php
    ...
    if ($fam_ID != "") {
      ...
      echo gettext(" dari <br> Kelg.") . " <a href=\"FamilyView.php?FamilyID=" . $fam_ID . "&amp;GID=$refresh\">" . $fam_Name . "<img border=0 src=\"Images/Icons/ico_kel.png\" align=\"middle\"  width=\"30\" height=\"30\"  ></a>" . gettext(" ") . "</font><br><br>";
    }
    else
...
//         imagedestroy($srcImage);
        ...
        // Calculate Resize Foto height and width (a "maxpect" algorithm)
        $dst_max_w = 200;
        $dst_max_h = 300;
        if ($src_w > $dst_max_w) {
          $thumb_w=$dst_max_w;
        ...
        $dstImage=ImageCreateTrueColor($thumb_w,$thumb_h);
            imagecopyresampled($dstImage,$srcImage,0,0,0,0,$thumb_w,$thumb_h,$src_w,$src_h);
        imagejpeg($dstImage, "Images/Person/" . $iPersonID . ".jpg");
        imagedestroy($dstImage);
          imagedestroy($srcImage);
    ...
    $photoFile1 = 'streamimg.php?rest=PERTHB126128'.$iPersonID;
    $photoFile2 = 'streamimg.php?res='.enkrip(PER,HRS,400,600,$iPersonID);    
 ...
 $photoFile = "Images/Person/thumbnails/" . $iPersonID . ".jpg";
        if (file_exists($photoFile))
        {
            ...
            echo '<img border="1" src="'.$photoFile1.'"  Images/NoPhoto.gif" width="128" height="126"   ></a>';

         //asli   echo '<a target="_blank" href="Images/Person/' . $iPersonID . '.jpg?'.$refresh.'">';
         //asli   echo '<img border="1" src="'.$photoFile.'?'.$refresh.'"  width="128" height="150" id="'.$refresh.'" ></a>';
            //echo '<img border="1" src="'.$photoFile.'?v='.$random.'"  width="128" height="150"  ></a>';
			...
			if ($bOkToEdit) {
                echo '
                    ...
                    value="' . gettext("Hapus Photo") . '" name="DeletePhoto">
                    </form>';
                }
        } else {
...
// Upload Surat Baptis Anak
        if ( isset($_POST["UploadSuratBaptisAnak"]) && ($_SESSION['bAddRecords'] || $bOkToEdit) ) {
          ...
          //  imagedestroy($dstImage);
            ...
            $dstImage=ImageCreateTrueColor($thumb_w,$thumb_h);
                imagecopyresampled($dstImage,$srcImage,0,0,0,0,$thumb_w,$thumb_h,$src_w,$src_h);
            imagejpeg($dstImage, "Images/BaptisAnak/SBA" . $iPersonID . ".jpg",60);
            imagedestroy($dstImage);
              imagedestroy($srcImage);
		...
		echo '<a target="_blank" href="Images/BaptisAnak/SBA' . $iPersonID . '.jpg"  >';
			...
			onsubmit="return confirmDelete()">
                        <input type="submit" class="icTinyButton"
                        value="' . gettext("X Hapus X") . '" name="DeleteSuratBaptisAnak">
                        </form>';
                    }
            } else {
                // Some old / M$ browsers can't handle PNG's correctly.
                if ($bDefectiveBrowser)
                    echo '<img border="0" src="Images/NoSurat.gif"  width="3" height="4"  >';
                else
                    echo '<img border="0" src="Images/NoSurat.png"  width="3" height="4"  >';
                ...
                if ($bOkToEdit) {
                    ...
                    echo '
                        ...
                        action="PersonView.php?PersonID=' . $iPersonID . '"
                        enctype="multipart/form-data">
                        <input class="icTinyButton" type="file" name="SuratBaptisAnak" size="1">
                        <input type="submit" class="icTinyButton"
                        value="' . gettext("Upload") . '" name="UploadSuratBaptisAnak">
                        </form>';
                }
        }
...
//              imagedestroy($srcImage);
            ...
            $dstImage=ImageCreateTrueColor($thumb_w,$thumb_h);
                imagecopyresampled($dstImage,$srcImage,0,0,0,0,$thumb_w,$thumb_h,$src_w,$src_h);
            imagejpeg($dstImage, "Images/BaptisDewasa/SBD" . $iPersonID . ".jpg");
            imagedestroy($dstImage);
              imagedestroy($srcImage);
...
//    move_uploaded_file($_FILES['SuratBaptisDewasa']['tmp_name'], "Images/BaptisDewasa/SBD" . $iPersonID . ".jpg");
		...
		echo "Surat Baptis Dewasa : <br>";  
            if (file_exists($SuratBaptisDewasaFile))
            {
		
                echo '<a target="_blank" href="Images/BaptisDewasa/SBD' . $iPersonID . '.jpg"  >';
                echo '<img border="1" src="'.$SuratBaptisDewasaFile.'"  width="60" height="80"  ></a>';
                if ($bOkToEdit) {
                    echo '
                        <form method="post"
                        action="PersonView.php?PersonID=' . $iPersonID . '">
                        <input type="submit" class="icTinyButton"
                        value="' . gettext("X Hapus X") . '" name="DeleteSuratBaptisDewasa">
                        </form>';
                    }
            } else {
                // Some old / M$ browsers can't handle PNG's correctly.
                ...
                if ($bOkToEdit) {
                    ...
                    echo '
                        ...
                        action="PersonView.php?PersonID=' . $iPersonID . '"
                        enctype="multipart/form-data">
                        <input class="icTinyButton" type="file" name="SuratBaptisDewasa" size="1">
                        <input type="submit" class="icTinyButton"
                        value="' . gettext("Upload") . '" name="UploadSuratBaptisDewasa">
                        </form>';
                }
        }
...
//      imagedestroy($srcImage);
  ...
  // Calculate foto's height and width (a "maxpect" algorithm)
  $dst_max_w = 800;
  $dst_max_h = 1100;
  if ($src_w > $dst_max_w) {
    $thumb_w=$dst_max_w;
  ...
  if ($dst_max_w >= $dst_max_h) {
    ...
    $dstImage=ImageCreateTrueColor($thumb_w,$thumb_h);
           imagecopyresampled($dstImage,$srcImage,0,0,0,0,$thumb_w,$thumb_h,$src_w,$src_h);
    imagejpeg($dstImage, "Images/Sidhi/SS" . $iPersonID . ".jpg");
    imagedestroy($dstImage);
    imagedestroy($srcImage);
    ...
    $SuratSidhiFile = "Images/Sidhi/thumbnails/SS" . $iPersonID . ".jpg";
		echo "Surat Sidhi : <br>";                
          if (file_exists($SuratSidhiFile))
          {
              ...
              echo '<img border="1" src="'.$SuratSidhiFile.'"  width="60" height="80"  ></a>';
              if ($bOkToEdit) {
                  echo '
                      <form method="post"
                      action="PersonView.php?PersonID=' . $iPersonID . '">
                      <input type="submit" class="icTinyButton"
                      value="' . gettext("X Hapus X") . '" name="DeleteSuratSidhi">
                      </form>';
                    }
            } else {
                // Some old / M$ browsers can't handle PNG's correctly.
                ...
                if ($bOkToEdit) {
                    ...
                    echo '
                        ...
                        action="PersonView.php?PersonID=' . $iPersonID . '"
                        enctype="multipart/form-data">
                        <input class="icTinyButton" type="file" name="SuratSidhi" size="1" >
                        <input type="submit" class="icTinyButton"
                        value="' . gettext("Upload") . '" name="UploadSuratSidhi">
                        </form>';
                }
        }

// Upload Surat Pengantar Pindah Attestasi
        if ( isset($_POST["UploadSuratPindah"]) && ($_SESSION['bAddRecords'] || $bOkToEdit) ) {
          if ($_FILES['SuratPindah']['name'] == "") {
            $SuratPindahError = gettext("Tidak Ada SuratPindah yang di upload.");
          } elseif ($_FILES['SuratPindah']['type'] != "image/pjpeg" && $_FILES['SuratPindah']['type'] != "image/jpeg") {
            $SuratPindahError = gettext("Hanya Format JPEG yang bisa di upload.");
          } else {
            // Create the thumbnail used by PersonView

                chmod ($_FILES['SuratPindah']['tmp_name'], 0777);

            $srcImage=imagecreatefromjpeg($_FILES['SuratPindah']['tmp_name']);
            $src_w=imageSX($srcImage);
              $src_h=imageSY($srcImage);
            ...
            $dstImage=ImageCreateTrueColor($thumb_w,$thumb_h);
                imagecopyresampled($dstImage,$srcImage,0,0,0,0,$thumb_w,$thumb_h,$src_w,$src_h);
            imagejpeg($dstImage, "Images/AtestasiMasuk/thumbnails/ATIN" . $iPersonID . ".jpg");
//            imagedestroy($dstImage);
//              imagedestroy($srcImage);
            ...
            $dstImage=ImageCreateTrueColor($thumb_w,$thumb_h);
                imagecopyresampled($dstImage,$srcImage,0,0,0,0,$thumb_w,$thumb_h,$src_w,$src_h);
            imagejpeg($dstImage, "Images/AtestasiMasuk/ATIN" . $iPersonID . ".jpg");
            imagedestroy($dstImage);
              imagedestroy($srcImage);


//    move_uploaded_file($_FILES['SuratPindah']['tmp_name'], "Images/AtestasiMasuk/ATIN" . $iPersonID . ".jpg");
          }
        } elseif (isset($_POST["DeleteSuratPindah"]) && $_SESSION['bDeleteRecords']) {
          unlink("Images/AtestasiMasuk/ATIN" . $iPersonID . ".jpg");
          unlink("Images/AtestasiMasuk/thumbnails/ATIN" . $iPersonID . ".jpg");
        }

        // Display SuratPindah or upload from file
        $SuratPindahFile = "Images/AtestasiMasuk/thumbnails/ATIN" . $iPersonID . ".jpg";
		echo "Surat Pindah / Atestasi : <br>";  
            if (file_exists($SuratPindahFile))
            {
		
                echo '<a target="_blank" href="Images/AtestasiMasuk/ATIN' . $iPersonID . '.jpg"  >';
                echo '<img border="1" src="'.$SuratPindahFile.'"  width="60" height="80"  ></a>';
                if ($bOkToEdit) {
                    echo '
                        <form method="post"
                        action="PersonView.php?PersonID=' . $iPersonID . '">
                        <input type="submit" class="icTinyButton"
                        value="' . gettext("X Hapus X") . '" name="DeleteSuratPindah">
                        </form>';
                    }
            } else {
                // Some old / M$ browsers can't handle PNG's correctly.
                ...
                if ($bOkToEdit) {
                    if (isset($SuratPindahError))
                        echo '<span style="color: red;">' . $SuratPindahError . '</span><br>';

                    echo '
                        <form method="post"
                        action="PersonView.php?PersonID=' . $iPersonID . '"
                        enctype="multipart/form-data">
                        <input class="icTinyButton" type="file" name="SuratPindah" size="1">
                        <input type="submit" class="icTinyButton"
                        value="' . gettext("Upload") . '" name="UploadSuratPindah">
                        </form>';
                }
        }

// Upload Kartu Tanda Penduduk
        if ( isset($_POST["UploadKTP"]) && ($_SESSION['bAddRecords'] || $bOkToEdit) ) {
          if ($_FILES['KartuTandaPenduduk']['name'] == "") {
            $KartuTandaPendudukError = gettext("Tidak Ada KartuTandaPenduduk yang di upload.");
          } elseif ($_FILES['KartuTandaPenduduk']['type'] != "image/pjpeg" && $_FILES['KartuTandaPenduduk']['type'] != "image/jpeg") {
            $KartuTandaPendudukError = gettext("Hanya Format JPEG yang bisa di upload.");
          } else {
            // Create the thumbnail used by PersonView

                chmod ($_FILES['KartuTandaPenduduk']['tmp_name'], 0777);

            $srcImage=imagecreatefromjpeg($_FILES['KartuTandaPenduduk']['tmp_name']);
            $src_w=imageSX($srcImage);
              $src_h=imageSY($srcImage);
            ...
            $dstImage=ImageCreateTrueColor($thumb_w,$thumb_h);
                imagecopyresampled($dstImage,$srcImage,0,0,0,0,$thumb_w,$thumb_h,$src_w,$src_h);
            imagejpeg($dstImage, "Images/KartuTandaPenduduk/thumbnails/KTP" . $iPersonID . ".jpg");
//            imagedestroy($dstImage);
//              imagedestroy($srcImage);
            ...
            $dstImage=ImageCreateTrueColor($thumb_w,$thumb_h);
                imagecopyresampled($dstImage,$srcImage,0,0,0,0,$thumb_w,$thumb_h,$src_w,$src_h);
            imagejpeg($dstImage, "Images/KartuTandaPenduduk/KTP" . $iPersonID . ".jpg");
            imagedestroy($dstImage);
              imagedestroy($srcImage);


//    move_uploaded_file($_FILES['KartuTandaPenduduk']['tmp_name'], "Images/KartuTandaPenduduk/KTP" . $iPersonID . ".jpg");
          }
        } elseif (isset($_POST["DeleteKartuTandaPenduduk"]) && $_SESSION['bDeleteRecords']) {
          unlink("Images/KartuTandaPenduduk/KTP" . $iPersonID . ".jpg");
          unlink("Images/KartuTandaPenduduk/thumbnails/KTP" . $iPersonID . ".jpg");
        }

        // Display KartuTandaPenduduk or upload from file
        $KartuTandaPendudukFile = "Images/KartuTandaPenduduk/thumbnails/KTP" . $iPersonID . ".jpg";
		echo "Kartu Tanda Penduduk : <br>";  
            if (file_exists($KartuTandaPendudukFile))
            {
		
                echo '<a target="_blank" href="Images/KartuTandaPenduduk/KTP' . $iPersonID . '.jpg"  >';
                echo '<img border="1" src="'.$KartuTandaPendudukFile.'"  width="80" height="60"  ></a>';
                if ($bOkToEdit) {
                    echo '
                        <form method="post"
                        action="PersonView.php?PersonID=' . $iPersonID . '">
                        <input type="submit" class="icTinyButton"
                        value="' . gettext("X Hapus X") . '" name="DeleteKartuTandaPenduduk">
                        </form>';
                    }
            } else {
                // Some old / M$ browsers can't handle PNG's correctly.
                if ($bDefectiveBrowser)
                    echo '<img border="0" src="Images/NoSurat.gif"  width="4" height="3"  >';
                else
                    echo '<img border="0" src="Images/NoSurat.png"  width="4" height="3"  >';

                if ($bOkToEdit) {
                    if (isset($KartuTandaPendudukError))
                        echo '<span style="color: red;">' . $KartuTandaPendudukError . '</span><br>';

                    echo '
                        <form method="post"
                        action="PersonView.php?PersonID=' . $iPersonID . '"
                        enctype="multipart/form-data">
                        <input class="icTinyButton" type="file" name="KartuTandaPenduduk" size="1">
                        <input type="submit" class="icTinyButton"
                        value="' . gettext("Upload") . '" name="UploadKTP">
                        </form>';
                }
        }		
...
<td width="80%" valign="top" align="left">
  ...
  <tr>
    <td align="center">
      ...
      <tr>
      <td class="TinyLabelColumn"><?php echo gettext("No.Reff.Gereja Asal"); ?></td>
      <td class="TinyTextColumn"><?php echo $per_LastName; ?></td>
      </tr>

      <tr>
      <td class="TinyLabelColumn"><?php 
	  if ($per_WorkPhone==""){echo "<b><blink><font color=\"red\">";echo gettext("Kelompok");echo "</font></blink></b>"; }else{echo gettext("Kelompok");}?></td>
      <td class="TinyTextColumn"><?php 
	  if ($per_WorkPhone==""){echo "<b><blink><font color=\"red\">";echo gettext("ERROR !!!");echo "</font></blink></b>"; }else{echo $per_WorkPhone;}?></td>
      </tr>
...
<?php } ?>
      ...
      <tr>
        ...
        <td class="TinyTextColumn"><?php echo $sClassName; 
		if ($sClassName == "Titipan" AND  $AsalTitipan == "" ){ echo "<font color=\"red\"><blink>Error:Asal Gereja masih Kosong!</blink></font>" ; }
		?></td>
      </tr>
      <?php
...
//          echo "<td class=\"TinyTextColumn\">" . displayCustomField($type_ID, $currentData, $custom_Special) . "</td></tr>";
  ...
  <b><?php echo gettext("Terdaftar di Kelompok:"); ?></b>

  <script language="javascript">
  function GroupRemove( Group, Person ) {
  var answer = confirm (<?php echo "'" . gettext("PERHATIAN: anda akan menghapus keanggotanan warga dari suatu kelompok") . "'"; ?>)
  if ( answer )
  ...
  {
    echo "<p align=\"center\">" . gettext("Belum terdaftar dalam kelompok.") . "</p>";
  }
  else
  {
    echo "<table width=\"100%\" cellpadding=\"4\" cellspacing=\"0\">";
    echo "<tr class=\"TableHeader\">";
    echo "<td>" . gettext("Nama Kelompok") . "</td>";
    echo "<td>" . gettext("Peran") . "</td>";
    if ($_SESSION['bManageGroups'])
    {
      echo "<td>" . gettext("Keterangan") . "</td>";
      echo "<td width=\"10%\">" . gettext("Hapus") . "</td>";
    }
    echo "</tr>";
    ...
    while ($aRow = mysql_fetch_array($rsAssignedGroups))
    {
      ...
      echo "<td><a href=\"GroupView.php?GroupID=" . $grp_ID . "&GID=$refresh&\">" . $grp_Name . " - " . $grp_Description . "</a></td>";

      echo "<td>";
      if ($_SESSION['bManageGroups']) echo "<a href=\"MemberRoleChange.php?GroupID=" . $grp_ID . "&PersonID=" . $iPersonID . "&GID=$refresh&\">";
      echo $roleName;
      ...
      if ($_SESSION['bManageGroups']) {
        if ($grp_hasSpecialProps == 'true')
          ...
          echo "<td>&nbsp;</td>";
        echo "<td><input type=\"button\" class=\"icTinyButton\" value=\"" . gettext("Hapus Kelompok") . "\" Name=\"remove\" onclick=\"GroupRemove(" . $grp_ID . ", " . $iPersonID . ");\" ></td>";
      }
      echo "</tr>";
  ...
  <p class="SmallText" align="center">
    <span class="SmallText"><?php echo gettext("Kelompok:"); ?></span>
    ...
    <input type="submit" class="icButton" <?php echo 'value="' . gettext("Tambahkan") . '"'; ?> name="GroupAssign" style="font-size: 8pt;">
    <br>
    <span class="SmallText" align="center"><?php echo gettext(" "); ?></span>
  </p>
  </form>
  <?php } ?>
  ...
  <p class="SmallText" align="center">
    <span class="SmallText"><?php echo gettext("Ciri ciri khusus:"); ?></span>
    ...
    <input type="submit" class="icButton" <?php echo 'value="' . gettext("Tambahkan") . '"'; ?> name="Submit" style="font-size: 8pt;">
  </p>
  </form>
  <?php }
  else
  ...
  else
  {
    echo "<table width=\"100%\" cellpadding=\"4\" cellspacing=\"0\">";
    echo "<tr class=\"TableHeader\">";
    echo "<td>" . gettext("Nama Pelayanan") . "</td>";
    echo "<td>" . gettext("Keterangan") . "</td>";
    ...
    while ($aRow = mysql_fetch_array($rsAssignedVolunteerOpps))
    {
      ...
      if ($_SESSION['bEditRecords']) echo "<td><a class=\"SmallText\" href=\"PersonView.php?PersonID=" . $per_ID . "&RemoveVO=" . $vol_ID . "&GID=$refresh&\">" . gettext("Hapus") . "</a></td>";
  ...
  <form method="post" action="PersonView.php?PersonID=<?php echo $iPersonID ?>">
  <p class="SmallText" align="center">
    <span class="SmallText"><?php echo gettext("Pelayanan :"); ?></span>
    ...
    <input type="submit" class="icButton" <?php echo 'value="' . gettext("Tambahkan") . '"'; ?> name="VolunteerOpportunityAssign" style="font-size: 8pt;">
    <br>
  </p>
  </form>
  <?php } ?>
