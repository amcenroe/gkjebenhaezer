...
function ValidateInput()
{
	...
	while ($aRow = mysql_fetch_array($rsParameters))
	{
		...
		{
			$bError = true;
			$aErrorText[$qrp_Alias] = gettext("Diperlukan Nilai / Isian pada Kolom ini.");
		}
		...
		else
		{
			...
			switch ($qrp_Validation)
			{
				...
				case "n":
					...
					{
						$bError = true;
						$aErrorText[$qrp_Alias] = gettext("Nilainya harus ANGKA.");
					}
					else
					{
						...
						{
							$bError = true;
							$aErrorText[$qrp_Alias] = gettext("Berisi Setidaknya ") . $qrp_NumericMin;
						}
						//Is it less than the maximum?
						elseif ($_POST[$qrp_Alias] > $qrp_NumericMax)
						{
							$bError = true;
							$aErrorText[$qrp_Alias] = gettext("Tidak Boleh lebih dari ") . $qrp_NumericMax;
						}
					}
				...
				case "a":
					...
					{
						$bError = true;
						$aErrorText[$qrp_Alias] = gettext("Berisi Setidaknya ") . $qrp_AlphaMaxLength . gettext(" characters long");
					}
					//is the length more than the minimum?
					elseif (strlen($_POST[$qrp_Alias]) < $qrp_AlphaMinLength)
					{
						$bError = true;
						$aErrorText[$qrp_Alias] = gettext("Tidak Boleh lebih dari ") . $qrp_AlphaMinLength . gettext(" characters long");
					}
...
function DisplayRecordCount()
{
	...
	if ($qry_Count == 1)
	{
		...
		echo "<p align=\"center\">";
		echo mysql_num_rows($rsQueryResults) . gettext(" data");
		echo "</p>";
	}
}
...
function DoQuery()
{
	...
	DisplayRecordCount();
		...
		echo "<p align=\"center\">";
		echo "<b>Query ID : " . $iQueryID . " - " . $qry_Name . "</b><br>";
		echo "<i>Keterangan :" . $qry_Description . "</i><br>";
		echo "</p>";
	...
	if (is_array($aHiddenFormField) && count($aHiddenFormField) > 0)
	{
		...
		<form method="post" action="CartView.php"><p align="center">
			...
			<input type="submit" class="icButton" name="AddToCartSubmit" value="<?php echo gettext("Tambahkan Hasil di Keranjang");?>">&nbsp;
			<input type="submit" class="icButton" name="AndToCartSubmit" value="<?php echo gettext("Interseksi Data di Keranjang");?>">&nbsp;
			<input type="submit" class="icButton" name="NotToCartSubmit" value="<?php echo gettext("Hapus Data di Keranjang");?>">
		</p></form>
		<?php
	}

	echo "<p align=\"center\"><a href=\"QueryView.php?QueryID=" . $iQueryID . "\">" . gettext("Jalankan Process Query lagi") . "</a></p>";
	echo "<p align=\"center\"><a href=\"QueryList.php\">". gettext("Kembali ke menu") . "</a></p>";
...
function DisplayParameterForm()
{
	...
	<td colspan="3" align="center">
		<br>
		<input class="icButton" type="Submit" value="<?php echo gettext("Jalankan Proses Query"); ?>" name="Submit">
	</p>
