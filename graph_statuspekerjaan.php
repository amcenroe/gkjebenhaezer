	   ...
	   pada grafik ada data yang disiapkan */
		...
		// Some data
				...
				$sSQL = "select count(a.per_ID) as Jumlah
						...
						WHERE a.per_ID = b.per_ID AND b.c3 = c.lst_OptionID AND c.lst_ID = 17 AND c.lst_OptionID >0
...
//				while ($hasil=mysql_fetch_array($perintah)){
	...
	//	 print_r($dataArray);
		...
		// Some data
				...
				$sSQL = "select c.lst_OptionName as Pekerjaan
						from person_per a , person_custom b , list_lst c
						WHERE a.per_ID = b.per_ID AND b.c3 = c.lst_OptionID AND c.lst_ID = 17 AND c.lst_OptionID >0
						GROUP BY c.lst_OptionName ORDER by lst_OptionSequence";
				$perintah = mysql_query($sSQL);
...
//	$graph->title->Set("Prosentasi Status Pekerjaan");
