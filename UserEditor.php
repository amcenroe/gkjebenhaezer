...
if (isset($_POST['save']) && $iPersonID > 0) {
	...
	// New user without person selected yet
		...
		$sSQL = "SELECT * FROM person_per LEFT JOIN user_usr ON person_per.per_ID = user_usr.usr_per_ID WHERE usr_per_ID IS NULL ORDER BY per_FirstName";
...
$sPageTitle = gettext('Editor Pengguna');
...
?>
	...
	<tr>
		<td class="LabelColumn"><?php echo gettext('Jemaat yang akan didaftar sebagai Pengguna:'); ?></td>
		<td class="TextColumn">
	...
	while ($aRow =mysql_fetch_array($rsPeople)) {
		extract($aRow);
	?>
				<option value="<?php echo $per_ID; ?>"<?php if ($per_ID == $iPersonID) { echo ' selected'; } ?>><?php echo $per_FirstName . ', ' .  $per_LastName; ?></option>
	<?php }	?>
			</select>
		</td>
	</tr>
...
?>
    ...
    <table cellpadding="4" align="center">
	<tr>
		<td class="LabelColumn"><?php echo gettext('Pengguna:'); ?></td>
...
<?php /*  removed until 1.3
	...
	<tr>
		...
		<td class="TextColumn"><input type="checkbox" name="Admin" value="1"<?php if ($usr_Admin) { echo ' checked'; } ?>>&nbsp;<span class="SmallText"><?php echo gettext('(Hak Akses Penuh.)'); ?></span></td>
	</tr>

	<tr>
		<td class="LabelColumn"><?php echo gettext('Gaya:'); ?></td>
		<td class="TextColumnWithBottomBorder"><select name="Style"><?php StyleSheetOptions('Include',$usr_Style); ?></select></td>
	</tr>

	<tr>
		<td colspan="2"><div align="center"><?php echo gettext('Catatan: Perubahan akan efektif setelah login berikutnya .'); ?></div></td>
	...
	<tr>
		...
		<input type="submit" class="icButton" <?php echo 'value="' . gettext('Simpan') . '"'; ?> name="save">&nbsp;<input type="button" class="icButton" name="Cancel" <?php echo 'value="' . gettext('Batal') . '"'; ?> onclick="javascript:document.location='UserList.php';">
		</td>
	</tr>
...
// Table Headings
echo '<br><br>';
echo 'Hak Akses jika dipilih TRUE maka Pengguna diberi hak akses untuk merubah nilainya.<br>';
echo '<table cellpadding="3" align="left">';
echo "\n".'<tr><td><h3>Hak Akses</h3></td>
    <td><h3>'. gettext("Nama Nilai") . '</h3></td>
	<td><h3>Nilai Sekarang </h3></td>
	<td><h3>Catatan</h3></td></tr>';
...
while ($aDefaultRow = mysql_fetch_row($rsDefault)) {
	...
	if ($r == 13) {
		echo "\n".'<tr><td>&nbsp;</td>
			<td><input type="submit" class="icButton" name="save" value="'.gettext('Simpan').'">
			<input type="submit" class="icButton" name="cancel" value="' . gettext('Batal') . '">
			</td></tr>';
		$r = 1;
	}
