...
<center><b><?php echo gettext("Peringatan: perubahan akan mempengaruhi data yang ada!"); ?></b><br><br>
...
if ( $bErrorFlag )
{
	...
	if ($bDuplicateFound) echo "<br>" . gettext("Salah: Duplikasi") . " " . $adj . " " . $noun . gettext("tidak diperbolehkan.");
...
<?php if ($mode == 'groupcustom' || $mode == 'custom' || $mode == 'famcustom') { ?>
	<input type="button" class="icButton" <?php echo 'value="' . gettext("Keluar") . '"'; ?> Name="Exit" onclick="javascript:window.close();">
<?php } elseif ($mode != "grproles") { ?>
	<input type="button" class="icButton" <?php echo 'value="' . gettext("Keluar") . '"'; ?> Name="Exit" onclick="javascript:document.location='<?php
	...
	<tr align="center">
		...
		<td class="TextColumn">
			...
			if ( $aNameErrors[$row] == 1 )
				echo "<span style=\"color: red;\"><BR>" . gettext("Masukkan nama.") . " </span>";
			elseif ( $aNameErrors[$row] == 2 )
				echo "<span style=\"color: red;\"><BR>" . gettext("Nama Duplikat!") . " </span>";
			?>
		</td>
		<?php
		if ($mode == "grproles")
...
<br>
<center>
Nama <?php echo $noun . " " . gettext(":"); ?>&nbsp;
<span class="SmallText">
	<input type="text" name="newFieldName" size="30" maxlength="40">
</span>
&nbsp;
<input type="submit" class="icTinyButton" <?php echo 'value="' . gettext("Tambahkan") . ' ' . $noun . ' ' . $adj . '"'?> Name="AddField">
<?php
	if ($bNewNameError > 0)
	{
		echo "<div><span style=\"color: red;\"><BR>";
		if ( $bNewNameError == 1 )
			echo gettext("SALAH: Anda belum memasukkan nama.");
		else
			echo gettext("SALAH: ") . $noun . gettext(" tersebut sudah ada.");
		echo "</span></div>";
	}
?>
</center>
