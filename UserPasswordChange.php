...
if (isset($_POST["Submit"]))
{
	...
	if ($bAdminOtherUser)
	{
		...
		if (strlen($sNewPassword1) == 0 && strlen($sNewPassword2) == 0) {
			$sNewPasswordError = "<br><font color=\"red\">" . gettext("Anda harus memasukkan password dua kali") . "</font>";
			$bError = True;
		}

		// Do the two new passwords match each other?
		elseif ($sNewPassword1 != $sNewPassword2) {
			$sNewPasswordError = "<br><font color=\"red\">" . gettext("Anda harus memasukkan password yang sama") . "</font>";
			$bError = True;
		}
	...
	else
	{
		...
		if (!$bPasswordMatch) {
			$sOldPasswordError = "<br><font color=\"red\">" . gettext("Password Anda Salah") . "</font>";
			$bError = True;
		}
		...
		elseif (in_array($sNewPassword1, $aBadPasswords)) {
			$sNewPasswordError = "<br><font color=\"red\">" . gettext("Password anda mudah ditebak. Silahkn pilih yang lain.") . "</font>";
			$bError = True;
		}

		// Is the password valid for length?
		elseif (strlen($sNewPassword1) < $sMinPasswordLength) {
			$sNewPasswordError = "<br><font color=\"red\">" . gettext("Password Anda harus minimal") . $sMinPasswordLength . gettext("karakter") . "</font>";
			$bError = True;
		}

		// Did they actually change their password?
		elseif ($sNewPassword1 == $sOldPassword) {
			$sNewPasswordError = "<br><font color=\"red\">" . gettext("Anda harus mengganti password anda") . "</font>";
			$bError = True;
		}

		elseif (levenshtein($sNewPassword1,$sOldPassword) < $sMinPasswordChange) {
			$sNewPasswordError = "<br><font color=\"red\">" . gettext("Password anda sama dengan password sebelumya, silahkan diganti") . "</font>";
			$bError = True;
		}
...
$sPageTitle = gettext("Perubahan User Password");
...
if (!$bAdminOtherUser)
	echo "<p>" . gettext("Silahkan masukkan password awal anda, kemudian masukkan password baru anda dua kali. Panjang Password harus minimum ") . ' ' . $sMinPasswordLength . ' ' . gettext("karakter.") . "</p>";
else
	echo "<p>" . gettext("Silahkan masukkan password untuk User ini.") . "</p>";
?>
...
<?php } ?>
	...
	<tr>
		<td colspan="2" align="center"><input type="submit" class="icButton" name="Submit" value="<?php echo gettext("Simpan"); ?>"></td>
	</tr>
</table>
</form>
