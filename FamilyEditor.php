...
if (isset($_POST["FamilySubmit"]) || isset($_POST["FamilySubmitAndAdd"]))
{
	...
	if ($sCountry == "Indonesia" || $sCountry == "Indonesia")
...
//	}
	...
	//	if (!$bNoFormat_WorkPhone) $sWorkPhone = CollapsePhoneNumber($sWorkPhone,$sCountry);
		...
		$logvar1 = "Edit";
		$logvar2 = "Family Added or Updated";
		//Update Logger
		$sSQL = "INSERT INTO logger (date, userid, fam_id, var1, var2)
			VALUES ('" . date("YmdHis") . "'," . $_SESSION['iUserID'].",'" . $iFamilyID . "','" . $logvar1 . "','" . $logvar2 . "')";
		//Execute the SQL
		...
		if (isset($_POST["FamilySubmit"]))
		{
			...
			Redirect("FamilyView.php?FamilyID=" . $iFamilyID."&amp;GID=$refresh");
		} else {
...
//Get NamaKelompok for the drop-down
$sSQL = "SELECT * FROM group_grp LEFT JOIN list_lst ON grp_Type = lst_OptionID WHERE lst_ID='3' OR grp_Type = 0 ORDER BY lst_OptionSequence, grp_Name ";
...
<table cellpadding="3" align="center">
	...
	<tr>
		<td colspan="2" align="center">
			...
			<input type="button" class="icButton" value="<?php echo gettext("Batal"); ?>" 
			
			name="FamilyCancel" onclick="javascript:document.location='<?php if (strlen($iFamilyID) > 0) 
			{ echo "FamilyView.php?FamilyID=".$iFamilyID."&amp;GID=$refresh"; 
			} else {echo "SelectList.php?mode=Family&amp;GID=$refresh"; 
			} ?>';">	
	</td>
	</tr>
	...
	<table cellpadding="3">
		...
		<tr>
			...
			<td class="TextColumn">
					<select name="WorkPhone" >
						<option value="" selected><?php echo gettext("Tidak Ada Data"); ?></option>
						<?php
						while ($aRow = mysql_fetch_array($rsNamaKelompok))
						{
							extract($aRow);

							echo "<option value=\"" . $grp_Name . "\"";
							if ($sWorkPhone == $grp_Name) { echo " selected"; }
							echo ">" . $grp_Name." - ".$grp_Description;
						}
						?>
					</select>
				...
				<?php //require "Include/KelompokDropDown.php"; ?>
			</td>
		</tr>
		<tr>
			<td class="LabelColumn"><?php echo gettext("Status Keluarga:"); ?></td>
			<td class="TextColumn">
				<?php require "Include/StKelgDropDown.php"; ?>
			</td>
		</tr>
	...
	<tr>
		...
		<td class="TextColumn">
			<?php require "Include/PropinsiDropDown.php"; ?>
		</td>
	</tr>
	...
	<tr>
		<td class="LabelColumn"><?php echo gettext("Telp.Rumah:"); ?></td>
		<td class="TextColumn">
...
<?php } ?>
	...
	if (!$iFamilyID)
	{
		...
		if (mysql_num_rows($rsProperties) > 0)
		{
			...
			echo "<tr><td class='LabelColumn'>" . gettext("Keterangan Khusus :") . "</td>\n";
			echo "<td class='TextColumnWithBottomBorder'>";
			// Display all family properties
			echo "<select name='PropertyID'><option value='0'>Tidak Ada Data</option>";
...
<tr>
		<td class="LabelColumn"><?php echo gettext("Koordinat Lat/Long:"); ?></td>
		<td class="TextColumnWithBottomBorder">
		<input type="text" Name="Latitude" value="<?php echo $nLatitude; ?>" size="15" maxlength="30">
		<input type="text" Name="Longitude" value="<?php echo $nLongitude; ?>" size="15" maxlength="30"></td>
	</tr>
...
<?php } /* Lat/Lon can be hidden - General Settings */ ?>
	...
	<tr>
		<td colspan="2">
		<div class="MediumText"><center><?php if (!strlen($iFamilyID)) { echo gettext(" Anda Dapat menambahkan anggota keluarga sekarang atau menambahkannya nanti."); }?></center></div><br><br>
		...
		<tr class="TableHeader" align="center">
			<td><?php echo gettext("Anggota Kelg"); ?></td>
		...
		**/ ?>
		...
		<tr>
			...
			<td class="TextColumn">
				<input name="FirstName<?php echo $iCount ?>" type="text" value="<?php echo $aFirstNames[$iCount] ?>" size="25">
		...
		**/ ?>
			<td class="TextColumn">
				<select name="Gender<?php echo $iCount ?>">
					<option value="0" <?php if ($aGenders[$iCount] == 0) echo "selected" ?> ><?php echo gettext("Pilih"); ?></option>
					<option value="1" <?php if ($aGenders[$iCount] == 1) echo "selected" ?> ><?php echo gettext("Laki-laki"); ?></option>
					<option value="2" <?php if ($aGenders[$iCount] == 2) echo "selected" ?> ><?php echo gettext("Perempuan"); ?></option>
				</select>
			</td>

			<td class="TextColumn">
				<select name="Role<?php echo $iCount ?>">
					<option value="0" <?php if ($aRoles[$iCount] == 0) echo "selected" ?> ><?php echo gettext("Peran"); ?></option>
				<?php
			...
			<td class="TextColumn">
				<select name="BirthMonth<?php echo $iCount ?>">
					<option value="0" <?php if ($aBirthMonths[$iCount] == 0) { echo "selected"; } ?>><?php echo gettext(" - "); ?></option>
					<option value="01" <?php if ($aBirthMonths[$iCount] == 1) { echo "selected"; } ?>><?php echo gettext("Januari"); ?></option>
					<option value="02" <?php if ($aBirthMonths[$iCount] == 2) { echo "selected"; } ?>><?php echo gettext("Februari"); ?></option>
					<option value="03" <?php if ($aBirthMonths[$iCount] == 3) { echo "selected"; } ?>><?php echo gettext("Maret"); ?></option>
					<option value="04" <?php if ($aBirthMonths[$iCount] == 4) { echo "selected"; } ?>><?php echo gettext("April"); ?></option>
					<option value="05" <?php if ($aBirthMonths[$iCount] == 5) { echo "selected"; } ?>><?php echo gettext("Mei"); ?></option>
					<option value="06" <?php if ($aBirthMonths[$iCount] == 6) { echo "selected"; } ?>><?php echo gettext("Juni"); ?></option>
					<option value="07" <?php if ($aBirthMonths[$iCount] == 7) { echo "selected"; } ?>><?php echo gettext("Juli"); ?></option>
					<option value="08" <?php if ($aBirthMonths[$iCount] == 8) { echo "selected"; } ?>><?php echo gettext("Agustus"); ?></option>
					<option value="09" <?php if ($aBirthMonths[$iCount] == 9) { echo "selected"; } ?>><?php echo gettext("September"); ?></option>
					<option value="10" <?php if ($aBirthMonths[$iCount] == 10) { echo "selected"; } ?>><?php echo gettext("Oktober"); ?></option>
					<option value="11" <?php if ($aBirthMonths[$iCount] == 11) { echo "selected"; } ?>><?php echo gettext("November"); ?></option>
					<option value="12" <?php if ($aBirthMonths[$iCount] == 12) { echo "selected"; } ?>><?php echo gettext("Desember"); ?></option>
				</select>
			</td>
			<td class="TextColumn">
			...
			<td>
				<select name="Classification<?php echo $iCount ?>">
					<option value="0" <?php if ($aClassification[$iCount] == 0) echo "selected" ?>><?php echo gettext("Klasifikasi"); ?></option>
