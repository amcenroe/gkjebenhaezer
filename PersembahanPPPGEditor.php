...
/*******************************************************************************
 *
 *  filename    : PersembahanPPPGEditor.php
...
//Set the page title
$sPageTitle = gettext("Editor Persembahan PPPG");

//Get the PersembahanPPPGID out of the querystring
$iPersembahanPPPGID = FilterInput($_GET["PersembahanPPPGID"],'int');
...
// Clean error handling: (such as somebody typing an incorrect URL ?PersembahanPPPGID= manually)
if (strlen($iPersembahanPPPGID) > 0)
{
...
if (isset($_POST["PersembahanSubmit"]) || isset($_POST["PersembahanSubmitAndAdd"]))
{
	...
	$sPersembahanPPPGID = FilterInput($_POST["PersembahanPPPGID"]);
	...
	$sKetExternal = FilterInput($_POST["KetExternal"]);
	$sPos = FilterInput($_POST["Pos"]);
	$sJmlAmplop = FilterInput($_POST["JmlAmplop"]);
	...
	//If no errors, then let's update...
		// New Data (add)
		if (strlen($iPersembahanPPPGID) < 1)
		{
			 	
			if (!$_SESSION['bFinance'] || strlen($sEnvelope) < 1)
				$sEnvelope = "NULL";

			$sSQL = "INSERT INTO PersembahanPPPG ( 	
		...
		//	echo $sSQL;
			...
			$logvar1 = "Edit";
			$logvar2 = "New Daftar Persembahan PPPG";
	...
	//update the Baptis table
			...
			$sSQL = "UPDATE PersembahanPPPG SET 
			...
			Bulan2 = '" . $sBulan2 . "',
			Bulanan = '" . $sBulanan . "',
			KetExternal = '" . $sKetExternal . "',
			Pos = '" . $sPos . "',
			JmlAmplop = '" . $sJmlAmplop . "',
	
			DateLastEdited = '" . date("YmdHis") . "',
			EditedBy = '" . $_SESSION['iUserID'] ;
				
			$sSQL .= "' WHERE PersembahanPPPGID = " . $iPersembahanPPPGID;
		...
		//	echo $sSQL;
			...
			$logvar1 = "Edit";
			$logvar2 = "Update Daftar Persembahan PPPG";
		}
		...
		$sSQL = "INSERT INTO logger (date, userid, per_id, var1, var2)
			VALUES ('" . date("YmdHis") . "'," . $_SESSION['iUserID'].",'" . $iPersembahanPPPGID . "','" . $logvar1 . "','" . $logvar2 . "')";
		//Execute the SQL
			RunQuery($sSQL);

		// Check for redirection to another page after saving information: (ie. PersembahanPPPGEditor.php?previousPage=prev.php?a=1;b=2;c=3)
		if ($previousPage != "") {
			$previousPage = str_replace(";","&",$previousPage) ;
			Redirect($previousPage . $iPersembahanPPPGID);
		}
		else if (isset($_POST["PersembahanSubmit"]))
		{
			//Send to the view of this PAK
			Redirect("SelectListApp3.php?mode=PersembahanPPPG&amp;$refresh");

		}
		else
		{
			//Reload to editor to add another record
			Redirect("PersembahanPPPGEditor.php");
		}
	...
	if (strlen($iPersembahanPPPGID) > 0)
	{
		...
		$sSQL = "SELECT * FROM PersembahanPPPG  WHERE PersembahanPPPGID = " . $iPersembahanPPPGID;
		$rsBaptis = RunQuery($sSQL);
		extract(mysql_fetch_array($rsBaptis));
		
		$sPersembahanPPPGID = $PersembahanPPPGID;
		$sTanggal = $Tanggal;
		$sPukul = $Pukul;
		$sKodeTI = $KodeTI;
		$sKelompok = $Kelompok;
		...
		$sBulan2 = $Bulan2;
		$sBulanan = $Bulanan;
		$sKetExternal = $KetExternal;
		$sPos = $Pos;
		$sJmlAmplop = $JmlAmplop;
...
$sSQL = "SELECT * FROM JenisPPPG ORDER BY KodeJenis";
$rsJenisPPPG = RunQuery($sSQL);
...
<form method="post" action="PersembahanPPPGEditor.php?PersembahanPPPGID=<?php echo $iPersembahanPPPGID; ?>" name="BacaanAkitabEditor">
...
<table cellpadding="3" align="center" valign="top" border="0">
	...
	<tr>
		<td <?php if ($numCustomFields > 0) echo "colspan=\"2\""; ?> align="center">
			...
			<input type="button" class="icButton" value="<?php echo gettext("Batal"); ?>" name="pakCancel" onclick="javascript:document.location='<?php if (strlen($iPersembahanPPPGID) > 0) 
{ echo "SelectListApp3.php?mode=PersembahanPPPG&amp;GID=$refresh"; 
} else {echo "SelectListApp3.php?mode=PersembahanPPPG&amp;GID=$refresh"; 
} ?>';">
		</td>
	</tr>
	...
	<tr>
		<td class="LabelColumn"><?php echo gettext("Jenis Persembahan PPPG:"); ?></td>
				<td colspan="0" class="TextColumn">
					<select name="Pos">
						<option value="0" selected><?php echo gettext("Tdk diketahui/lain-lain"); ?></option>
						<?php
						while ($aRow = mysql_fetch_array($rsJenisPPPG))
						{
							extract($aRow);
		...
		<td class="LabelColumn"><?php echo gettext("Jumlah Amplop (*Amplop Biru):"); ?></td>
		<td class="TextColumn" colspan="0"><input type="text" size=25 name="JmlAmplop" id="JmlAmplop" value="<?php echo htmlentities(stripslashes($sJmlAmplop),ENT_NOQUOTES, "UTF-8"); ?>"><br><font color="red"><?php echo $sJmlAmplopError ?></font></td>
	...
	<tr>	
				...
				<td>
						...
						<td class="LabelColumn"><?php echo gettext("Keterangan *Jika Bukan Warga:"); ?></td>
		<td class="TextColumn" colspan="4"><input type="text" size=50 name="KetExternal" id="KetExternal" value="<?php echo htmlentities(stripslashes($sKetExternal),ENT_NOQUOTES, "UTF-8"); ?>"><br><font color="red"><?php echo $sKetExternalError ?></font></td>
