...
require "Include/FunctionsMove.php";
...
elseif (isset($_SESSION['SelectListMoveMode']))
	$sMode = $_SESSION['SelectListMoveMode'];

switch ($sMode)
	{
	case ('groupassign'):
		$_SESSION['SelectListMoveMode'] = $sMode;
		break;
	case ('family'):
		$_SESSION['SelectListMoveMode'] = $sMode;
		break;
	default:
		$_SESSION['SelectListMoveMode'] = 'person';
		break;
	}
...
if ($sMode == 'person')
{
    $sPageTitle = gettext("Daftar Jemaat Pindah / Meninggal");
    $iMode = 1;
...
if ($iMode == 1 || $iMode == 2)
{
    ...
    if ($iMode == 2)
    {
    	$sBaseSQL = "SELECT *, IF(LENGTH(per_Zip) > 0,per_Zip,fam_Zip) AS zip " .
					"FROM person_pindah NATURAL JOIN person_custom_pindah LEFT JOIN family_pindah " .
					"ON person_pindah.per_fam_ID = family_pindah.fam_ID  ";
        ...
        $sSQLsub =	"SELECT person_pindah.per_ID FROM NATURAL JOIN person_custom_pindah person_pindah LEFT JOIN person2group2role_p2g2r " .
					"ON p2g2r_per_ID = person_pindah.per_ID LEFT JOIN group_grp " .
					"ON grp_ID = p2g2r_grp_ID " .
					"WHERE grp_Type = $iGroupTypeMissing GROUP BY person_pindah.per_ID";
        ...
        if (mysql_num_rows($rsSub) > 0)
        {
            ...
            $sGroupWhereExt = " AND person_pindah.per_ID NOT IN (" . $sExcludedIDs . ")";
        }
 	}
    ...
    if ($iMode == 1)
    {
       ...
       $sBaseSQL =	"SELECT *, IF(LENGTH(per_Zip) > 0,per_Zip,fam_Zip) AS zip  " .
					"FROM person_pindah NATURAL JOIN person_custom_pindah LEFT JOIN family_pindah " .
					"ON per_fam_ID = family_pindah.fam_ID ";
		...
		if (isset($iGroupType))
		{
			if ($iGroupType >= 0)
			{
				$sJoinExt = " LEFT JOIN person2group2role_p2g2r ON person_pindah.per_ID = p2g2r_per_ID ".
				...
				if (isset($iGroupID))
					if ($iGroupID >= 0)
					{
                        if (isset($iRoleID))
                        {
                            $sJoinExt = " LEFT JOIN person2group2role_p2g2r ".
                                        " ON person_pindah.per_ID = p2g2r_per_ID ".
                                        " LEFT JOIN list_lst ".
                                        " ON p2g2r_grp_ID = lst_ID ";
                            $sGroupWhereExt =   " AND p2g2r_grp_ID=".$iGroupID." " .
                                                " AND p2g2r_per_ID= person_pindah.per_ID " .
					...
					else
					{
						$sJoinExt =	" LEFT JOIN person2group2role_p2g2r ".
									...
									" LEFT JOIN group_grp ON grp_ID = p2g2r_grp_ID ";
						$sGroupWhereExt =	" AND grp_type=".$iGroupType." ".
											" AND person_pindah.per_ID NOT IN ".
			...
			{
				$sJoinExt = " ";
				$sGroupWhereExt =	" AND person_pindah.per_ID NOT IN (SELECT p2g2r_per_ID ".
		...
		if (isset($iPersonProperty)) {
			...
			{
				$sJoinExt2 = " LEFT JOIN record2property_r2p ON person_pindah.per_ID = r2p_record_ID "; // per_ID should match the r2p_record_ID
				$sPersonPropertyWhereExt =	" AND r2p_pro_ID = ".$iPersonProperty." ";
			}
			else // >>>> THE SQL CODE BELOW IS NOT TESTED PROPERLY <<<<<
			{
				$sJoinExt2 = " ";
				$sPersonPropertyWhereExt =	" AND person_pindah.per_ID NOT IN (SELECT r2p_record_ID ".
...
//code asli         $sLetterWhereExt = " AND per_LastName LIKE '" . $sLetter . "%'";
	...
	$sGroupBySQL = " GROUP BY person_pindah.per_ID";

     $sWhereExt =	$sGroupWhereExt . $sFilterWhereExt . $sClassificationWhereExt .
					$sFamilyRoleWhereExt . $sGenderWhereExt . $sLetterWhereExt . $sPersonPropertyWhereExt . $sPersonGolDarahWhereExt . $sPersonStatusKawinWhereExt  ;

	$sSQL = $sBaseSQL . $sJoinExt . " WHERE 1" . $sWhereExt . $sGroupBySQL;


	// URL to redirect back to this same page
	$sRedirect = "SelectListMove.php?";
    ...
    if (!$bPrintView)
    {
		...
		$sSQL = "SELECT DISTINCT LEFT(per_FirstName,1) AS letter FROM person_pindah NATURAL JOIN person_custom_pindah ".
//code asli $sSQL = "SELECT DISTINCT LEFT(per_LastName,1) AS letter FROM person_pindah NATURAL JOIN person_custom_pindah ".
//		$sSQL = "SELECT DISTINCT LEFT(per_FirstName,1) AS letter FROM person_pindah ".
        ...
        echo "<form method=\"get\" action=\"SelectListMove.php\" name=\"PersonList\">";
        ...
        if ($iMode == 1)
        {
            ...
            echo "<a href=\"SelectListMove.php?mode=$sMode&amp;type=$iGroupTypeMissing&amp;Filter=$sFilter&amp;Classification=$iClassification&amp;FamilyRole=$iFamilyRole&amp;Gender=$iGender&amp;grouptype=$iGroupType&amp;groupid=$iGroupID&amp;grouproleid=$iRoleID&amp;PersonGolDarah=$iPersonGolDarah&amp;PersonStatusKawin=$iPersonStatusKawin&amp;";
            if($sSort)
				echo "&amp;Sort=$sSort";
		...
		?>
				...
				if (!isset($iGender))
					echo " selected ";
				echo '> ' . gettext ("Laki-laki & Perempuan") . '</option>';
				...
				if (isset($iGroupID) && ($iGroupID > -1))
				{
					...
					echo '</select>';
                        ...
                        <input type="button" class="icButton" value="<?php echo gettext("Hilangkan Filter"); ?>" onclick="javascript:document.location='SelectListMove.php?mode=<?php echo $sMode; ?>&amp;Sort=<?php echo $sSort; ?>&amp;type=<?php echo $iGroupTypeMissing; ?>'"><BR><BR>
<?php
...
?>                        </td></tr>
                ...
                echo "<a href=\"SelectListMove.php?mode=$sMode&amp;type=$iGroupTypeMissing&amp;Filter=$sFilter&amp;Classification=$iClassification&amp;FamilyRole=$iFamilyRole&amp;Gender=$iGender&amp;grouptype=$iGroupType&amp;groupid=$iGroupID&amp;grouproleid=$iRoleID&amp;PersonGolDarah=$iPersonGolDarah&amp;PersonStatusKawin=$iPersonStatusKawin&amp";
                ...
                {
                        echo "<div align=\"center\">";
                        echo "<form method=\"get\" action=\"SelectListMove.php\" name=\"ListNumber\">";

                        // Show previous-page link unless we're at the first page
                        if ($Result_Set < $Total && $Result_Set > 0)
                        {
                                $thisLinkResult = $Result_Set - $iPerPage;
                                echo "<a href=\"SelectListMove.php?Result_Set=$thisLinkResult&amp;mode=$sMode&amp;type=$iGroupTypeMissing&amp;Filter=$sFilter&amp;Sort=$sSort&amp;Letter=$sLetter&amp;Classification=$iClassification&amp;FamilyRole=$iFamilyRole&amp;Gender=$iGender&amp;grouptype=$iGroupType&amp;groupid=$iGroupID&amp;grouproleid=$iRoleID&amp;PersonGolDarah=$iPersonGolDarah&amp;PersonStatusKawin=$iPersonStatusKawin&amp\">". gettext("Halaman Sebelumnya") . "</A>&nbsp;&nbsp;";
                        }
                        ...
                        if ($startpage != 1)
                                echo "<a href=\"SelectListMove.php?Result_Set=0&amp;mode=$sMode&amp;type=$iGroupTypeMissing&amp;Filter=$sFilter&amp;Sort=$sSort&amp;Letter=$sLetter&amp;Classification=$iClassification&amp;FamilyRole=$iFamilyRole&amp;Gender=$iGender&amp;grouptype=$iGroupType&amp;groupid=$iGroupID&amp;grouproleid=$iRoleID&amp;PersonGolDarah=$iPersonGolDarah&amp;PersonStatusKawin=$iPersonStatusKawin&amp\">1</a> ... \n";
                        ...
                        if ($Pages > 1)
                        {
                                for ($c = $startpage; $c <= $endpage; $c++)
                                {
                                        ...
                                        if ($thisLinkResult != $Result_Set)
                                                echo "&nbsp;&nbsp;<a href=\"SelectListMove.php?Result_Set=$thisLinkResult&amp;mode=$sMode&amp;type=$iGroupTypeMissing&amp;Filter=$sFilter&amp;Sort=$sSort&amp;Letter=$sLetter&amp;Classification=$iClassification&amp;FamilyRole=$iFamilyRole&amp;Gender=$iGender&amp;grouptype=$iGroupType&amp;groupid=$iGroupID&amp;grouproleid=$iRoleID&amp;PersonGolDarah=$iPersonGolDarah&amp;PersonStatusKawin=$iPersonStatusKawin&amp\">$c</a>&nbsp;\n";
                                        else
                                                echo "&nbsp;&nbsp;[ " . $c . " ]&nbsp;&nbsp;";
                                }
                        }
                        ...
                        if ($endpage != $Pages)
                        {
                                ...
                                echo " ... <a href=\"SelectListMove.php?Result_Set=$thisLinkResult&amp;mode=$sMode&amp;type=$iGroupTypeMissing&amp;Filter=$sFilter&amp;Sort=$sSort&amp;Letter=$sLetter&amp;Classification=$iClassification&amp;FamilyRole=$iFamilyRole&amp;Gender=$iGender&amp;grouptype=$iGroupType&amp;groupid=$iGroupID&amp;grouproleid=$iRoleID&amp;PersonGolDarah=$iPersonGolDarah&amp;PersonStatusKawin=$iPersonStatusKawin&amp\">$Pages</a>\n";
                        }
                        ...
                        if ($Result_Set >= 0 && $Result_Set < $Total)
                        {
                                ...
                                if ($thisLinkResult<$Total)
                                        echo "&nbsp;&nbsp;<a href=\"SelectListMove.php?Result_Set=$thisLinkResult&amp;mode=$sMode&amp;type=$iGroupTypeMissing&amp;Filter=$sFilter&amp;Sort=$sSort&amp;Letter=$sLetter&amp;Classification=$iClassification&amp;FamilyRole=$iFamilyRole&amp;Gender=$iGender&amp;grouptype=$iGroupType&amp;groupid=$iGroupID&amp;grouproleid=$iRoleID&amp;PersonGolDarah=$iPersonGolDarah&amp;PersonStatusKawin=$iPersonStatusKawin&amp\">" . gettext("Halaman Berikutnya") . "</a>";
                        }
...
// At this point we have finished the forms at the top of SelectListMove.
...
echo '<form method="get" action="SelectListMove.php" name="ColumnOptions">';
...
echo '<td><a href="SelectListMove.php?mode=' .$sMode. '&amp;type=' .$iGroupTypeMissing;
...
//			echo "<td class=\"ControlBreak\">" . mb_strtoupper(mb_substr($per_LastName,0,1,"UTF-8"));
	...
	echo "<td><a href=\"PersonViewMove.php?PersonID=" .$per_ID. "\">";
...
// 	if ($sPersonColumn3 == "GolDarah")
//		 		echo $aPersonGolDarahName[$c6];
	...
	if ($fam_Name != "")
	{
		echo "<a href=\"FamilyViewMove.php?FamilyID=" . $fam_ID . "\">" . $fam_Name;
...
//
	...
	if ($iMode == 1)
	{
		...
		echo "<td><a href=\"PrintView.php?PersonID=" .$per_ID. "\" target=\"_blank\">";
...
else
{
        ...
        // Base SQL
        $sSQL = "SELECT * FROM family_pindah";
        ...
        $sSQL = "SELECT DISTINCT LEFT(fam_Name,1) AS letter FROM family_pindah ORDER BY letter";
        $rsLetters = RunQuery($sSQL);

        //Does this user have AddModify permissions?
        if ($_SESSION['bAddRecords']) { echo "<div align=\"center\"><a href=\"FamilyEditor.php\">" . gettext("Tambahkan Data Keluarga Baru") . "</a></div><BR>"; }
        ?>
        <form method="get" action="SelectListMove.php" name="FamilyList">
                <p align="center">
                <?php echo gettext("Diurutkan berdasarkan:"); ?>
                <select name="Sort">
        ...
        // Create Sort Links
        echo "<div align=\"center\">";
        echo "<a href=\"SelectListMove.php?mode=family\">" . gettext("Lihat Semua") . "</a>";
        while ($aLetter = mysql_fetch_array($rsLetters))
        {
                echo "&nbsp;&nbsp;|&nbsp;&nbsp;<a href=\"SelectListMove.php?mode=family";
                if($sSort) echo "&amp;Sort=$sSort";
                echo "&amp;Letter=" . $aLetter[0] . "\">" . $aLetter[0] . "</a>";
        }
        ...
        if ($Total > 0)
        {
                ...
                {
                        $thisLinkResult = $Result_Set - $iPerPage;
                        echo "<A HREF=\"SelectListMove.php?Result_Set=$thisLinkResult&amp;mode=family&amp;Filter=$sFilter&amp;Sort=$sSort&amp;Letter=$sLetter\">Previous Page</A>&nbsp;&nbsp;";
                }
                ...
                // Show Link "1 ..." if startpage does not start at 1
                if ($startpage != 1)
                        echo "&nbsp;&nbsp;<a href=\"SelectListMove.php?Result_Set=0&amp;mode=family&amp;Filter=$sFilter&amp;Sort=$sSort&amp;Letter=$sLetter\">1</a> ... \n";
                ...
                if ($Pages > 1)
                {
                        for ($c = $startpage; $c <= $endpage; $c++)
                        {
                                ...
                                $thisLinkResult = $iPerPage * $b;
                                if ($thisLinkResult != $Result_Set)
                                        echo "&nbsp;&nbsp;<a href=\"SelectListMove.php?Result_Set=$thisLinkResult&amp;mode=family&amp;Filter=$sFilter&amp;Sort=$sSort&amp;Letter=$sLetter\">$c</a>&nbsp;\n";
                                else
                                        echo "&nbsp;&nbsp;[ " . $c . " ]&nbsp;&nbsp;";
                        }
                }
                ...
                {
                        $thisLinkResult = ($Pages - 1) * $iPerPage;
                        echo " ... <a href=\"SelectListMove.php?Result_Set=$thisLinkResult&amp;mode=family&amp;Filter=$sFilter&amp;Sort=$sSort&amp;Letter=$sLetter\">$Pages</a>\n";
                }
                ...
                if ($Result_Set >= 0 && $Result_Set < $Total)
                {
                        $thisLinkResult=$Result_Set+$iPerPage;
                        if ($thisLinkResult<$Total)
                                echo "&nbsp;&nbsp;<a href=\"SelectListMove.php?Result_Set=$thisLinkResult&amp;mode=family&amp;Filter=$sFilter&amp;Sort=$sSort&amp;Letter=$sLetter\">" . gettext("Next Page") . "</a>";
                }
                ?>
        ...
        <?php //        <td><?php echo gettext("Diedit terakhir"); ?></td>  ?>
        </tr>
        ...
        while ($aRow = mysql_fetch_array($rsFamilies))
        {
                ...
                if ($bFamListFirstNames)
                {
                        ...
                        $sFirstNames = "";
                        $sSQL = "SELECT per_FirstName FROM person_pindah
			...
			$sBlankLine = "<tr><td>&nbsp;</td></tr>";
                ...
                <tr class="<?php echo $sRowClass ?>">
                        ...
                        <?php } ?>
                        <td><a href="FamilyViewMove.php?FamilyID=<?php echo $fam_ID ?>"><?php echo $fam_Name ?></a>&nbsp;</td>
