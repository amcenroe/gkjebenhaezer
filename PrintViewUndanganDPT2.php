...
/*******************************************************************************
 *
 *  filename    : PrintViewUndanganDPT2.php
 *
...
$ifamID = FilterInput($_GET["famID"],'int');
...
$iTema = FilterInput($_GET["Tema"],'int');
						...
						WHERE per_fam_ID = $ifamID
						AND (per_cls_id < 3) AND (( (c2 is not NULL AND c2<>'0000-00-00 00:00:00')
							AND c27 is not NULL )
...
?>	
  ...
  <tr>
    ...
    <?php
						...
						$NomorSurat =  "UND".$ifamID."e/K/DPT/MG/".$sChurchCode."/".dec2roman($month)."/".$year;
  ...
  // echo "<tr><td valign=top colspan=3 ><font size=\"2\"><b> " . $Sapaan . " " . $NamaLengkap . "</b></td><td><font size=\"2\"></font></td></tr>";
				while ($hasilGD=mysql_fetch_array($rsPerson))
				{
