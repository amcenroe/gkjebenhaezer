...
/*******************************************************************************
 *
 *  filename    : PrintViewLapPersembahanPPPG.php
...
$Judul = "Informasi Laporan Persembahan Kartu PPPG  ".date(Y,strtotime($iTGL)); 
require "Include/Header-Report.php";

$sSQL0="SELECT MONTH(MAX(Tanggal)) as Pembagi FROM PersembahanPPPG a WHERE Pos=1 AND  ".$sSQLTanggal5;
//echo $sSQL0;
...
//echo $Pembagi;
				...
				echo "<td align=\"left\">";
				echo "<a href=\"PrintViewLapPersembahanPPPG.php?TGL=".$yearago."&amp;GID=$refresh\"  >";
				...
				echo "<td align=\"right\">"; 
				echo "<a href=\"PrintViewLapPersembahanPPPG.php?TGL=".$nextyear."&amp;GID=$refresh\"  >";
...
// Detail Laporan
				...
				echo "<tr><td bgcolor=\"#0020C2\" colspan=\"16\" align=\"center\" ><b>
	...
	$sSQL1 = "SELECT DISTINCT(IFNULL(TRIM(Kelompok),0))as NamaKelompok FROM PersembahanPPPG ORDER BY Kelompok" ;
    ...
    $sRowClass = AlternateRowStyle($sRowClass); 
				...
				//extract($aRow);
					...
					// Data Persembahan Bulanan 
						$sSQL = "SELECT Kelompok, IFNULL(SUM(Bulanan),0) as PersBulanan FROM PersembahanPPPG a
							WHERE Pos=1 AND  Kelompok = '$NamaKelompok' AND MONTH(a.Tanggal) = ".$tbul1." ".$sSQLTanggal3;
					...
					<a href=\"PrintViewCekPersembahan.php?Bul=".$tbul1."&Cek=PPPG&TGL=".$iTGL."&Pos=1 \" target=\"_blank\"   STYLE=\"TEXT-DECORATION: NONE\">
			...
			while ($t1 < 12 )
				{
				$t1++;
				$sSQL = "SELECT IFNULL(SUM(Bulanan),0) as totperbulan FROM PersembahanPPPG a
							WHERE Pos=1 AND  MONTH(a.Tanggal) = ".$t1." ".$sSQLTanggal3;
							$rsPersembahan = RunQuery($sSQL);
							extract(mysql_fetch_array($rsPersembahan));
...
// Detail Laporan Jumlah Kartu
    ...
    $sRowClass = AlternateRowStyle($sRowClass); 
				...
				//extract($aRow);
					...
					// Data Persembahan Bulanan 
						$sSQL = "SELECT Kelompok, IFNULL(COUNT(Kelompok),0) as PersBulanan FROM PersembahanPPPG a
							WHERE Pos=1 AND  Kelompok = '$NamaKelompok' AND MONTH(a.Tanggal) = ".$tbul1." ".$sSQLTanggal3;
							$rsPersembahan = RunQuery($sSQL);
							extract(mysql_fetch_array($rsPersembahan));
			...
			while ($t1 < 12 )
				{
				$t1++;
				$sSQL = "SELECT IFNULL(COUNT(Kelompok),0) as totperbulan FROM PersembahanPPPG a
							WHERE Pos=1 AND  MONTH(a.Tanggal) = ".$t1." ".$sSQLTanggal3;
							$rsPersembahan = RunQuery($sSQL);
							extract(mysql_fetch_array($rsPersembahan));
